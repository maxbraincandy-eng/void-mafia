...
function executeDeclare(io, match, player, tricks, trump) {
...
export function registerJokerHandlers(io, socket) {
    ...
    // ── Create match ──────────────────────────────────────────────────
    socket.on('joker:create', async (data, cb) => {
        try {
    ...
    // ── Join match ────────────────────────────────────────────────────
    socket.on('joker:join', async (data, cb) => {
        try {
            const name = String(data?.name ?? 'Player').trim().slice(0, 24) || 'Player';
            ...
            if (existingPlayer) {
                ...
                socket.join(JOKER_ROOM(match.id));
                socket.emit('joker:hand', match.hands[existingPlayer.id] ?? []);
                broadcastState(io, match);
