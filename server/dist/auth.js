...
async function upsertAuthAccount(userId, provider, providerUserId, email, displayName, avatarUrl) {
    const now = Date.now();
    await sql `
...
async function findOrCreateOAuthUser(provider, providerUserId, email, displayName, avatarUrl) {
...
async function linkProviderToUser(userId, provider, providerUserId, email, displayName, avatarUrl) {
...
export function createAuthRouter() {
    ...
    router.get('/facebook/callback', passport.authenticate('facebook', { session: false, failureRedirect: `${process.env.CLIENT_URL ?? ''}/?oauth_error=1` }), async (req, res) => {
        const CLIENT_URL = process.env.CLIENT_URL ?? '';
        try {
            const profile = req.user;
            const linkUid = req.session.linkUid;
    ...
    router.get('/linked', async (req, res) => {
        const uid = req.session.uid ?? req.query.uid;
        if (!uid) {
