...
export async function createTournament(createdBy, name, maxPlayers, prizeCoins) {
    const id = generateId();
    const now = Date.now();
...
export async function joinTournament(tournamentId, playerId) {
    const [t] = await sql `SELECT status, max_players FROM tournaments WHERE id = ${tournamentId}`;
    if (!t)
        throw new Error('Tournament not found.');
    if (t.status !== 'open')
        throw new Error('Tournament is not open for registration.');
    const [{ cnt }] = await sql `SELECT COUNT(*) as cnt FROM tournament_participants WHERE tournament_id = ${tournamentId}`;
    if (Number(cnt) >= t.max_players)
        throw new Error('Tournament is full.');
    const [existing] = await sql `SELECT 1 FROM tournament_participants WHERE tournament_id = ${tournamentId} AND player_id = ${playerId}`;
    if (existing)
...
export async function leaveTournament(tournamentId, playerId) {
    const [t] = await sql `SELECT status FROM tournaments WHERE id = ${tournamentId}`;
    if (!t || t.status !== 'open')
        throw new Error('Cannot leave after tournament started.');
    await sql `DELETE FROM tournament_participants WHERE tournament_id = ${tournamentId} AND player_id = ${playerId}`;
}
export async function startTournament(tournamentId, requesterId) {
    const [t] = await sql `SELECT created_by, status FROM tournaments WHERE id = ${tournamentId}`;
    if (!t)
        throw new Error('Tournament not found.');
...
export async function finishTournament(tournamentId, winnerId) {
...
export async function listOpenTournaments() {
    const rows = await sql `SELECT id FROM tournaments WHERE status IN ('open', 'in_progress') ORDER BY created_at DESC LIMIT 20`;
    const results = [];
    for (const r of rows) {
