...
export const PHOTO_MAX_CHARS = 2800000; // ≈ 2 MB per picture after compression
export const PHOTOS_MAX = 24;
export const VOICE_MAX_CHARS = 8400000; // ≈ 6 MB, about ten minutes of speech
...
function cleanYear(raw) {
    ...
    if (!Number.isFinite(n) || n < 1800)
        return null;
    const max = new Date().getUTCFullYear();
    return n > max ? null : n;
}
export async function addMedia(input) {
    const kind = input.kind === 'voice' ? 'voice' : 'photo';
    const data = String(input.data ?? '');
    if (kind === 'photo') {
        if (!PHOTO_RE.test(data))
            throw new Error('სურათის ფორმატი მიუღებელია (PNG/JPEG/WEBP).');
        if (data.length > PHOTO_MAX_CHARS)
            throw new Error('სურათი ძალიან დიდია.');
    }
    else {
        if (!VOICE_RE.test(data))
            throw new Error('აუდიოს ფორმატი მიუღებელია.');
        if (data.length > VOICE_MAX_CHARS)
            throw new Error('ჩანაწერი ძალიან გრძელია — მაქსიმუმ ~6 MB.');
    }
    const [{ n }] = await sql `
  ...
  `;
    ...
    const item = {
        ...
        durationMs: Math.max(0, Math.min(3600000, Math.trunc(Number(input.durationMs) || 0))),
        bytes: bytesOf(data),
...
export async function listMedia(subjectId, kind) {
    const rows = kind
        ? await sql `
...
export async function getMediaWithData(id) {
    const [r] = await sql `
...
export async function listMediaWithData(subjectId, kind) {
    const rows = kind
        ? await sql `SELECT * FROM mars_media WHERE subject_id = ${subjectId} AND kind = ${kind} ORDER BY sort, created_at`
        : await sql `SELECT * FROM mars_media WHERE subject_id = ${subjectId} ORDER BY kind, sort, created_at`;
    return rows.map(r => ({ ...rowToItem(r), data: r.data }));
}
export async function updateMediaCaption(id, caption, year) {
    await sql `
    UPDATE mars_media
...
export async function deleteMedia(id) {
    const rows = await sql `DELETE FROM mars_media WHERE id = ${id} RETURNING id`;
    return rows.length > 0;
}
/** Which record a media item belongs to — so permission is checked on it. */
export async function subjectIdOfMedia(id) {
    const [r] = await sql `SELECT subject_id FROM mars_media WHERE id = ${id}`;
...
export async function addEvent(input) {
    ...
    if (title.length < 2)
        throw new Error('დაწერე, რა მოხდა.');
    const [{ n }] = await sql `SELECT COUNT(*)::int AS n FROM mars_events WHERE subject_id = ${input.subjectId}`;
    if (Number(n) >= EVENTS_MAX)
        throw new Error(`მაქსიმუმ ${EVENTS_MAX} მოვლენა.`);
...
export async function listEvents(subjectId) {
...
export async function deleteEvent(id) {
    const rows = await sql `DELETE FROM mars_events WHERE id = ${id} RETURNING id`;
    return rows.length > 0;
}
export async function subjectIdOfEvent(id) {
    const [r] = await sql `SELECT subject_id FROM mars_events WHERE id = ${id}`;
    return r?.subject_id ?? null;
}
//# sourceMappingURL=marsMedia.js.map