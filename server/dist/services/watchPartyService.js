...
const codes = new Map(); // code → matchId
const MAX_CHAT = 120;
function genCode() {
...
export function parseSource(rawInput) {
    ...
    for (const re of YT_RE) {
        ...
        if (m)
            return { raw: href, provider: 'youtube', refId: m[1], title: 'YouTube video', synced: true };
    }
    const vm = href.match(VIMEO_RE);
    if (vm)
        return { raw: href, provider: 'vimeo', refId: vm[1], title: 'Vimeo video', synced: true };
    ...
    if (tv)
        return { raw: href, provider: 'twitch', refId: `video:${tv[1]}`, title: 'Twitch video', synced: false };
    ...
    if (tk)
        return { raw: href, provider: 'tiktok', refId: tk[1], title: 'TikTok', synced: false };
...
// ── Lifecycle ─────────────────────────────────────────────────────────
export function createMatch(hostId, socketId, name, avatar, title) {
    const id = generateId();
    const code = genCode();
    const now = Date.now();
    const host = { userId: hostId, socketId, name, avatar, joinedAt: now };
    const m = {
        id, code,
...
export function joinMatch(id, userId, socketId, name, avatar) {
...
export function leaveMatch(id, userId) {
    const m = matches.get(id);
    if (!m)
...
export function transferHost(id, hostId, targetUserId) {
    const m = matches.get(id);
...
export function findBySocket(socketId) {
    for (const m of matches.values()) {
        if (m.members.some(x => x.socketId === socketId))
            return m;
    }
    return undefined;
}
...
export function effectivePosition(m) {
    if (!m.playing)
        return m.positionSec;
    const elapsed = (Date.now() - m.updatedAt) / 1000;
    return m.positionSec + elapsed * m.rate;
}
function capture(m, positionSec, playing) {
    m.positionSec = Math.max(0, positionSec);
    m.playing = playing;
    m.updatedAt = Date.now();
}
export function setSource(id, hostId, rawUrl) {
    const m = matches.get(id);
    if (!m || m.hostId !== hostId)
        return null;
...
export function pause(id, hostId, positionSec) {
    const m = matches.get(id);
    if (!m || m.hostId !== hostId)
        return null;
...
export function seek(id, hostId, positionSec) {
    const m = matches.get(id);
    if (!m || m.hostId !== hostId)
        return null;
...
export function setRate(id, hostId, rate) {
    const m = matches.get(id);
    if (!m || m.hostId !== hostId)
        return null;
...
// ── Queue ─────────────────────────────────────────────────────────────
export function queueAdd(id, hostId, rawUrl) {
    const m = matches.get(id);
    if (!m || m.hostId !== hostId)
        return null;
    const src = parseSource(rawUrl);
    if (!src)
        return null;
    if (m.queue.length >= 50)
        return null;
    // If nothing is playing yet, load it straight away instead of queueing.
    if (!m.source) {
        m.source = src;
        capture(m, 0, src.synced);
        return m;
    }
    m.queue.push(src);
    return m;
}
export function queueRemove(id, hostId, index) {
    const m = matches.get(id);
    if (!m || m.hostId !== hostId)
        return null;
    if (index < 0 || index >= m.queue.length)
        return null;
    m.queue.splice(index, 1);
    return m;
}
export function queueNext(id, hostId) {
    const m = matches.get(id);
    if (!m || m.hostId !== hostId)
        return null;
...
// ── Chat ──────────────────────────────────────────────────────────────
export function addChat(id, userId, text) {
    const m = matches.get(id);
    if (!m)
        return null;
    ...
    if (!clean)
        return null;
    const msg = { id: generateId(), userId, name: member.name, text: clean, at: Date.now() };
    m.chat.push(msg);
    if (m.chat.length > MAX_CHAT)
        m.chat.splice(0, m.chat.length - MAX_CHAT);
    return { m, msg };
}
...
//# sourceMappingURL=watchPartyService.js.map