...
export const ROUND_MS = 240000; // 4 minutes to run the temple
export const DUEL_MS = 60000;
export const OVER_MS = 8000; // scoreboard before the next round
...
function log(m, text) {
    ...
    if (m.log.length > 60)
        m.log.splice(0, m.log.length - 60);
}
function mkPlayer(userId, socketId, nickname, seat) {
    return {
...
export function matchOfPlayer(userId) {
...
export function fireTrap(m, userId, trapId, cooldownMs) {
    if (m.status !== 'running')
        return { ok: false, error: 'რაუნდი არ მიმდინარეობს' };
    const p = m.players.find(x => x.userId === userId);
    if (!p || p.role !== 'death')
        return { ok: false, error: 'მხოლოდ სიკვდილს შეუძლია' };
    const now = Date.now();
    if ((m.trapCooldown[trapId] ?? 0) > now)
        return { ok: false, error: 'გაცივება' };
...
export function reportDeath(m, userId, cause) {
    if (m.status !== 'running' && m.status !== 'duel')
        return;
    const p = m.players.find(x => x.userId === userId);
    if (!p || !p.alive)
 ...
 */
export function swordHit(m, attackerId, victimId) {
    if (m.status !== 'duel')
        return null;
    const a = m.players.find(p => p.userId === attackerId);
    const v = m.players.find(p => p.userId === victimId);
    if (!a || !v || !a.alive || !v.alive)
        return null;
    if (!m.duellists.includes(attackerId) || !m.duellists.includes(victimId))
        return null;
    if (a.role === v.role)
        return null; // survivors don't hit each other
    v.hp--;
    if (v.hp > 0)
        return { dead: false, victim: v };
    v.alive = false;
    a.kills++;
...
export function duelResult(m) {
    ...
    if (!death || !death.alive)
        return 'runners';
    if (!survivors.some(s => s.alive))
        return 'death';
    return null;
}
export function endRound(m, winner, why) {
    if (m.status === 'over' || m.status === 'waiting')
        return;
    m.status = 'over';
...
export function move(m, userId, x, y, z, ry) {
    ...
    p.y = y;
    p.z = z;
    p.ry = Number.isFinite(ry) ? ry : 0;
}
// ── views ─────────────────────────────────────────────────────────────
export function getState(m) {
    return {
        id: m.id,
        code: m.code,
        status: m.status,
...
//# sourceMappingURL=deathrunService.js.map