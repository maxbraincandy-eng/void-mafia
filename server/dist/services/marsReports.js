...
export const REPORT_REASONS = ['alive', 'not_authorised', 'false_info', 'offensive', 'duplicate', 'other'];
...
export async function report(args) {
    const reason = REPORT_REASONS.includes(args.reason)
        ? args.reason : 'other';
    const note = String(args.note ?? '').slice(0, REPORT_NOTE_MAX);
    const [subject] = await sql `SELECT id, steward_id, player_id FROM mars_subjects WHERE id = ${args.subjectId}`;
    if (!subject)
  ...
  `;
    ...
    const [row] = await sql `SELECT * FROM mars_reports WHERE subject_id = ${args.subjectId} AND reporter_id = ${args.reporterId}`;
...
export async function reportCount(subjectId) {
    const [{ n }] = await sql `SELECT COUNT(*)::int AS n FROM mars_reports WHERE subject_id = ${subjectId} AND status = 'open'`;
    return Number(n);
}
/**
 * Resolve a report.
...
export async function resolveReport(reportId, action, moderatorId) {
    const [r] = await sql `SELECT subject_id FROM mars_reports WHERE id = ${reportId}`;
    if (!r)
        throw new Error('რეპორტი ვერ მოიძებნა.');
    const now = Date.now();
    if (action === 'remove') {
...
export async function restoreRecord(subjectId) {
    await sql `UPDATE mars_subjects SET hidden = false, hidden_reason = '' WHERE id = ${subjectId}`;
}
//# sourceMappingURL=marsReports.js.map