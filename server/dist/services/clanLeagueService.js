...
export async function getLeague(weekStart, limit = 25) {
...
export async function getClanLeagueDetail(clanId, weekStart) {
    const week = weekStart ?? weekStartMs(Date.now());
...
export async function getLeagueHistory(limit = 12) {
    const rows = await sql `
...
export async function getClanTrophies(clanId) {
    const [r] = await sql `
 ...
 */
export async function settleLeague(grant, notify, maxWeeks = 8) {
    ...
    for (let i = 1; i <= maxWeeks; i++) {
        ...
        const [any] = await sql `SELECT 1 AS x FROM clan_league_contrib WHERE week_start = ${week} LIMIT 1`;
        if (!any)
            continue;
    ...
    `;
      ...
      `;
            for (const m of members) {
                ...
                await notify?.(m.player_id, '🏆 კლანების ლიგა', `${clan.clanName} დაიკავა #${rank + 1} ადგილი — +${coins} მონეტა!`).catch(() => { });
                paidPlayers++;
            }
            await sql `
...
//# sourceMappingURL=clanLeagueService.js.map