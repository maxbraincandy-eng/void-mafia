...
const TONE_POINTS = { triumph: 500, survival: 300, ruin: 120, death: 40 };
const VALID_TONES = new Set(['triumph', 'survival', 'ruin', 'death']);
const clamp = (n, lo, hi) => Math.max(lo, Math.min(hi, Math.trunc(Number(n) || 0)));
/**
 ...
 * crowd the board.
 */
export async function submitRun(userId, name, sub) {
...
export async function myStats(userId) {
  ...
  `;
    return {
        ...
        endings: seen.map((r) => r.ending_id),
    };
}
//# sourceMappingURL=noirService.js.map