...
export async function spendOnLiveGift(senderId, hostId, gift, sessionId) {
    const [row] = await sql `
  ...
  `;
    ...
    const { balanceAfter } = await recordTransaction(senderId, 'gift_sent', -gift.price, `ეთერის საჩუქარი: ${gift.name}`, { relatedUserId: hostId, relatedGiftId: `live:${sessionId}:${gift.id}` });
    return {
 ...
 */
export async function creditLiveGifts(hostId, coins, sessionId) {
    if (!Number.isInteger(coins) || coins <= 0)
        return getCoins(hostId);
    const { balanceAfter } = await recordTransaction(hostId, 'gift_received', coins, `ეთერის საჩუქრები (${coins} ქოინი)`, { relatedGiftId: `live:${sessionId}` });
    return balanceAfter;
}
...
export async function creditStorePurchase(args) {
    ...
    if (!platform || !transactionId)
        throw new Error('platform and transactionId required.');
    if (!Number.isInteger(coins) || coins <= 0)
  ...
  `;
    ...
    const { balanceAfter } = await recordTransaction(profileId, 'grant', coins, `IAP ${platform} ${productId} (${transactionId})`);
