...
export async function getProfile(userId) {
    const [row] = await sql `SELECT * FROM logic_profiles WHERE user_id = ${userId}`;
    if (row)
  ...
  `;
    const [fresh] = await sql `SELECT * FROM logic_profiles WHERE user_id = ${userId}`;
 ...
 */
async function pickQuestions(userId, count, level) {
    const pool = level === 'mixed' ? ALL_QUESTIONS : BY_LEVEL[level];
    const seenRows = await sql `SELECT question_id FROM logic_seen WHERE user_id = ${userId}`;
...
function testPlan(count) {
    ...
    order.forEach((lv, i) => { for (let k = 0; k < Math.round(count * share[i]); k++)
        plan.push(lv); });
    while (plan.length < count)
        plan.push('medium');
    return plan.slice(0, count);
}
export async function startSession(userId, mode, level, count = 10) {
...
export function sessionView(s) { return viewOf(s); }
export async function answer(sessionId, userId, chosen, ms) {
    const s = sessions.get(sessionId);
    if (!s || s.userId !== userId || s.finished)
        return null;
    const cur = s.questions[s.idx];
    if (!cur || cur.answeredAt)
        return null;
    ...
    return {
        ...
        rule: showNow ? q.rule : '',
        why: showNow ? q.why : '',
        trap: showNow ? (q.trap?.[pos] ?? null) : null,
        explain: showNow,
        done,
        next: done ? null : viewOf(s),
    };
}
export async function finish(sessionId, userId) {
    const s = sessions.get(sessionId);
    if (!s || s.userId !== userId || s.finished)
        return null;
    s.finished = true;
    ...
    const rank = ['beginner', 'medium', 'hard', 'expert'];
    const hardestHere = levels.reduce((best, l) => (rank.indexOf(l) > rank.indexOf(best) ? l : best), (before.hardest || 'beginner'));
  ...
  `;
    ...
    if (s.mode === 'daily')
        ...
        coins = Math.round(correct * 1.5);
    // system grant: the academy itself is the payer, so owner and target match
    if (coins > 0) {
        try {
            await grantCoins(userId, userId, coins, 'ლოგიკის აკადემია');
        }
...
async function checkAchievements(userId, p) {
    const had = new Set((await sql `SELECT code FROM logic_achievements WHERE user_id = ${userId}`).map(r => r.code));
    const now = Date.now();
    const earned = [];
    for (const a of ACHIEVEMENTS) {
        if (had.has(a.code) || !a.test(p))
            continue;
...
export async function getAchievements(userId) {
    const rows = await sql `SELECT code, earned_at FROM logic_achievements WHERE user_id = ${userId}`;
...
export async function leaderboard(scope, userId, limit = 50) {
...
export async function myRanks(userId) {
    const [me] = await sql `SELECT country FROM players WHERE id = ${userId}`;
    const [prof] = await sql `SELECT rating, answered FROM logic_profiles WHERE user_id = ${userId}`;
    const [{ total }] = await sql `SELECT COUNT(*)::int AS total FROM logic_profiles WHERE answered > 0`;
    if (!prof || Number(prof.answered) === 0)
        return { world: null, country: null, countryCode: me?.country ?? null, totalPlayers: Number(total) };
...
export async function categoryBreakdown(userId) {
    const rows = await sql `SELECT question_id FROM logic_seen WHERE user_id = ${userId}`;
