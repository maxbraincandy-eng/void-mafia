...
function award(playerId, key) {
    const existing = db.prepare('SELECT 1 FROM player_achievements WHERE player_id = ? AND achievement_key = ?').get(playerId, key);
    if (existing)
        return false;
    db.prepare('INSERT OR IGNORE INTO player_achievements (player_id, achievement_key, earned_at) VALUES (?, ?, ?)').run(playerId, key, Date.now());
    return true;
}
/** Check and award achievements after a game ends. Returns list of newly earned keys. */
export function checkAchievements(room, playerId) {
    ...
    // First win
    if (win && award(profileId, 'first_blood'))
        newAchievements.push('first_blood');
    // Role-specific wins
    if (win && role === 'don' && award(profileId, 'godfather'))
        newAchievements.push('godfather');
    if (win && role === 'jester' && award(profileId, 'ghost'))
        newAchievements.push('ghost');
    if (win && role === 'maniac' && award(profileId, 'serial_killer'))
        newAchievements.push('serial_killer');
    if (win && role === 'arsonist' && award(profileId, 'arsonist_win'))
        newAchievements.push('arsonist_win');
    // Bodyguard — died protecting
    if (role === 'bodyguard' && !player.isAlive && award(profileId, 'loyal_guard'))
        newAchievements.push('loyal_guard');
    // Survival
    if (player.isAlive && win && award(profileId, 'survivor'))
        newAchievements.push('survivor');
    // Games played milestones — check stats from DB
    const row = db.prepare('SELECT games_played FROM players WHERE id = ?').get(profileId);
    const gamesPlayed = (row?.games_played ?? 0) + 1; // +1 because addGameResult runs after this
    if (gamesPlayed >= 10 && award(profileId, 'night_owl'))
        newAchievements.push('night_owl');
    if (gamesPlayed >= 50) {
        const winsRow = db.prepare('SELECT wins FROM players WHERE id = ?').get(profileId);
        const totalWins = (winsRow?.wins ?? 0) + (win ? 1 : 0);
        if (totalWins >= 50 && award(profileId, 'legend'))
