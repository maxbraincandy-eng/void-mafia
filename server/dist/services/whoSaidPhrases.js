 ...
 */
export function pickPhrase(used) {
 ...
 */
export function pickReader(players, previous) {
    ...
    let best = pool[0];
    for (const p of pool) {
        if (p.timesRead < best.timesRead || (p.timesRead === best.timesRead && p.seat < best.seat))
            best = p;
    }
    return best.userId;
}
 ...
 */
export function scoreRound(readerId, votes) {
    const points = {};
    const correct = [];
    let fooled = 0;
    for (const [voter, target] of Object.entries(votes)) {
        if (voter === readerId)
            continue; // the reader does not vote
        if (target === readerId) {
            correct.push(voter);
    ...
    if (fooled > 0)
        points[readerId] = (points[readerId] ?? 0) + fooled * POINTS_PER_FOOLED;
    return { correct, fooled, points };
}
//# sourceMappingURL=whoSaidPhrases.js.map