...
export async function adminGetUserProfile(playerId) {
...
// ── Issue a warning ──────────────────────────────────────────────────────
export async function issueWarning(playerId, issuedBy, reason) {
...
// ── Suspend a user ───────────────────────────────────────────────────────
export async function suspendUser(playerId, suspendedBy, reason, durationSeconds) {
    ...
    const expiresAt = durationSeconds === 0 ? 9999999999999 : Date.now() + durationSeconds * 1000;
    await sql `
...
// ── Lift suspension ──────────────────────────────────────────────────────
export async function liftSuspension(playerId) {
    await sql `UPDATE community_suspensions SET active = 0 WHERE player_id = ${playerId}`;
}
// ── Mute a user ──────────────────────────────────────────────────────────
export async function muteUser(playerId, mutedBy, reason, durationSeconds) {
...
// ── Unmute ───────────────────────────────────────────────────────────────
export async function unmuteUser(playerId) {
    await sql `UPDATE community_mutes SET active = 0 WHERE player_id = ${playerId}`;
}
// ── Check if user is suspended ───────────────────────────────────────────
export async function isUserSuspended(playerId) {
    const [row] = await sql `
...
export async function isUserCommunityMuted(playerId) {
    const [row] = await sql `
...
// ── Profile controls ─────────────────────────────────────────────────────
export async function setProfileControls(playerId, controls) {
...
// ── Soft-delete a post ───────────────────────────────────────────────────
export async function adminDeletePost(postId, deletedBy) {
...
export async function adminDeleteComment(commentId, deletedBy) {
...
export async function listAllReports(limit = 100) {
    return await sql `
    SELECT r.*,
...
export async function listDeletedContent(type, limit = 50) {
    if (type === 'posts') {
        return await sql `
