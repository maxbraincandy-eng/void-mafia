...
async function savePerks(profileId, perks) {
 ...
 */
export async function buyPerk(profileId, perkId) {
    const def = PERK_ITEMS[perkId];
    if (!def)
    ...
    if (balance < def.price)
        throw new Error(`არ გყოფნის მონეტა. საჭიროა ${def.price}, გაქვს ${balance}.`);
...
export async function resolveAnon(profileId) {
    if (!profileId)
        return false;
    const p = await getPerks(profileId);
 ...
 */
export async function consumeXpBoost(profileId) {
    if (!profileId)
        return false;
    const perks = await getPerks(profileId);
    if (perks.xpBoostGames <= 0)
        return false;
...
export async function resolveSpotlightUntil(profileId) {
    if (!profileId)
        return null;
    const p = await getPerks(profileId);
