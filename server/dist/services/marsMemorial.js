...
export const MEMORY_PHOTO_MAX_CHARS = 4000000;
const YEAR_MIN = 1850;
/** Validate and normalise the person's identity fields. */
export function sanitiseMemorial(input, nowYear) {
    ...
    if (personFirst.length < 2)
        throw new Error('სახელი აუცილებელია.');
    if (personLast.length < 2)
        throw new Error('გვარი აუცილებელია.');
    const year = (v) => {
        if (v == null || v === '')
            return null;
        ...
        if (n < YEAR_MIN || n > nowYear)
            throw new Error(`წელი უნდა იყოს ${YEAR_MIN}-დან ${nowYear}-მდე.`);
        return n;
    };
...
export async function addMemory(args) {
    ...
    if (text.length < MEMORY_MIN)
        throw new Error(`მოგონება ძალიან მოკლეა — მინიმუმ ${MEMORY_MIN} სიმბოლო.`);
    let photo = null;
    if (typeof args.photo === 'string' && args.photo !== '') {
        if (args.photo.length > MEMORY_PHOTO_MAX_CHARS)
    ...
    const [subject] = await sql `SELECT id FROM mars_subjects WHERE id = ${args.subjectId}`;
    if (!subject)
  ...
  `;
    const [row] = await sql `SELECT * FROM mars_memories WHERE id = ${id}`;
    return rowToMemory(row);
}
export async function listMemories(subjectId, limit = 50) {
...
export async function deleteMemory(memoryId, requesterId) {
    const [m] = await sql `SELECT author_id, subject_id FROM mars_memories WHERE id = ${memoryId}`;
    if (!m)
        return false;
    const [s] = await sql `SELECT player_id, steward_id FROM mars_subjects WHERE id = ${m.subject_id}`;
    const isSteward = s && (s.steward_id === requesterId || s.player_id === requesterId);
    if (m.author_id !== requesterId && !isSteward)
        throw new Error('უფლება არ გაქვს.');
 ...
 */
export function topicsOf(corpus, limit = 6) {
    const freq = new Map();
    for (const p of corpus) {
        for (const w of tokens(p.text)) {
            ...
            if (cur) {
                cur.n++;
                if (w.length > cur.word.length)
                    cur.word = w;
            }
            else
                freq.set(k, { n: 1, word: w });
        }
    }
    return [...freq.values()]
 ...
 */
export function findPassage(question, corpus) {
    const q = new Set(tokens(question));
    if (q.size === 0 || corpus.length === 0)
        return { passage: null, score: 0 };
    let best = null;
    let bestScore = 0;
    for (const p of corpus) {
...
export function buildCorpus(args) {
    const out = [];
    const push = (text, kind, author, at) => {
        for (const s of sentences(text))
            out.push({ text: s, sourceKind: kind, sourceAuthor: author, sourceAt: at });
    };
    ...
    for (const m of args.memories)
        push(m.text, 'memory', m.authorName, m.createdAt);
    return out;
}
/**
 ...
 */
export function speak(question, personName, corpus) {
