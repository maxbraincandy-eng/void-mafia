...
export async function getPetData(playerId) {
    const rows = await sql `SELECT pet_xp, pet_level FROM player_pets WHERE player_id = ${playerId}`;
    if (!rows.length) {
...
export async function addPetXp(playerId, amount) {
    await sql `
