...
import { PERK_ITEMS, getPerks, buyPerk, setPerkMode, setPerkChoice, resolveSpectatorInvisible, resolveAnon, consumeXpBoost, resolveSpotlightUntil, aliasFor, resolveEntrance, resolveRoomSkin, resolveVoiceMask, consumeSticker, consumePostBoost, POST_BOOST_MS, refundPostBoost, } from './services/perkService.js';
/** Accepted values for the perks:configure / perks:choose sockets. */
const TOGGLE_PERKS = ['invisible', 'anon', 'entrance', 'voicemask'];
const CHOICE_PERKS = ['entrance', 'roomskin', 'voicemask'];
...
const stickerTimes = new Map();
function stickerRateOk(socketId) {
    const now = Date.now();
    const times = (stickerTimes.get(socketId) ?? []).filter(t => now - t < 15000);
    if (times.length >= 6)
        return false;
...
import { startLive, endLive, beat as liveBeat, joinLive, leaveLive, forgetViewer, addHearts, getSession, listLive, liveMap, myLive, roomFor, } from './services/liveService.js';
 ...
 */
function shown(p) { return p.anonAlias ?? p.name; }
async function applySpectatorInvisibility(player, profileId) {
 ...
 */
async function applyIncognito(player, profileId, room, wanted) {
    if (!wanted)
        return false;
    ...
    try {
        if (!(await limitsFor(profileId)).incognito)
            return false;
    }
    catch {
        return false;
...
const _duelPending = new Map(); // targetSocketId → challengerSocketId (unanswered invite)
const _duelPendingTimer = new Map(); // targetSocketId → auto-expire timer
...
function _clearDuel(socketId) {
    ...
    const t1 = _duelPendingTimer.get(socketId);
    if (t1) {
        clearTimeout(t1);
        _duelPendingTimer.delete(socketId);
    }
    _duelPending.delete(socketId);
    for (const [target, challenger] of _duelPending) {
        if (challenger === socketId) {
            const t = _duelPendingTimer.get(target);
            if (t) {
                clearTimeout(t);
                _duelPendingTimer.delete(target);
            }
            _duelPending.delete(target);
        }
    }
}
const _rpsPending = new Map(); // targetSocketId → challengerSocketId
const _rpsPendingTimer = new Map();
const _rpsGames = new Map(); // key = `${a}:${b}` (sorted)
function _rpsKey(a, b) { return a < b ? `${a}:${b}` : `${b}:${a}`; }
function _clearRps(socketId) {
    const t1 = _rpsPendingTimer.get(socketId);
    if (t1) {
        clearTimeout(t1);
        _rpsPendingTimer.delete(socketId);
    }
    _rpsPending.delete(socketId);
    for (const [target, challenger] of _rpsPending) {
        if (challenger === socketId) {
            const t = _rpsPendingTimer.get(target);
            if (t) {
                clearTimeout(t);
                _rpsPendingTimer.delete(target);
            }
            _rpsPending.delete(target);
        }
    }
    for (const [key, g] of _rpsGames) {
        if (g.aSocketId === socketId || g.bSocketId === socketId)
            _rpsGames.delete(key);
    }
}
...
const _todPending = new Map();
const _todPendingTimer = new Map();
function _clearTod(socketId) {
    const t1 = _todPendingTimer.get(socketId);
    if (t1) {
        clearTimeout(t1);
        _todPendingTimer.delete(socketId);
    }
    _todPending.delete(socketId);
    for (const [target, challenger] of _todPending) {
        if (challenger === socketId) {
            const t = _todPendingTimer.get(target);
            if (t) {
                clearTimeout(t);
                _todPendingTimer.delete(target);
            }
            _todPending.delete(target);
        }
    }
}
const TOD_TRUTHS = [
...
const _reactionGames = new Map(); // spaceId → game
const _reactionLeaderboard = new Map(); // spaceId → (socketId → {name, wins})
function _clearReactionGame(spaceId) {
...
function _finishReaction(spaceId, io) {
    const g = _reactionGames.get(spaceId);
    if (!g)
        return;
    g.phase = 'done';
    ...
    if (g.results.length > 0) {
        ...
        if (!lb) {
            lb = new Map();
            _reactionLeaderboard.set(spaceId, lb);
        }
    ...
    io.to(`space:${spaceId}`).emit('space:reaction_result', { results: g.results, leaderboard });
...
const WORLD_MAX = 40;
const WORLD_IDS = new Set(['beach_camp', 'cyber_lounge', 'mountain_cabin', 'skyline_terrace', 'yacht_club', 'rotmundi', 'speedway']);
...
const _backroomsMimics = new Map(); // instanceId → active mimic
const _backroomsMimicTimers = new Map(); // instanceId → next-mimic timer
...
function _brHash3(x, z, s) {
    let h = (x | 0) * 374761393 + (z | 0) * 668265263 + s * 2147483647;
    h = Math.imul(h ^ (h >>> 13), 1274126177);
    h = h ^ (h >>> 16);
    return (h >>> 0) / 4294967295;
}
function _brCellSealed(cx, cz, seed) {
...
function _runVoidEvent(instanceId, io) {
    ...
    while (shelters.length < want && guard++ < 24) {
        ...
        if (_brCellSealed(scx, scz, seed))
            continue;
        shelters.push({ x: scx * BACKROOMS_CELL + BACKROOMS_CELL / 2, z: scz * BACKROOMS_CELL + BACKROOMS_CELL / 2 });
    }
    io.to(`backrooms:${instanceId}`).emit('backrooms:event', { kind: 'void_warning', shelters });
        ...
        seq.push(setTimeout(() => {
            ...
            for (const p of r3.values()) {
                ...
                if (shelters.some(s => (p.x - s.x) * (p.x - s.x) + (p.z - s.z) * (p.z - s.z) < SHELTER_R2)) {
                    io.to(p.socketId).emit('backrooms:event', { kind: 'void_spared' });
                    continue;
                }
...
// ── The Void Mimic ──────────────────────────────────────────────────────
function _ensureBackroomsMimic(instanceId, io) {
    if (_backroomsMimicTimers.has(instanceId))
        return;
    _scheduleBackroomsMimic(instanceId, io);
}
function _scheduleBackroomsMimic(instanceId, io) {
    ...
    const timer = setTimeout(() => {
        ...
        if (room.size >= 2 && !_backroomsMimics.has(instanceId))
            _spawnBackroomsMimic(instanceId, io);
        _scheduleBackroomsMimic(instanceId, io);
    }, delay);
    _backroomsMimicTimers.set(instanceId, timer);
}
function _despawnBackroomsMimic(instanceId, io) {
    const m = _backroomsMimics.get(instanceId);
    if (!m)
        return;
    clearInterval(m.tick);
    _backroomsMimics.delete(instanceId);
    io.to(`backrooms:${instanceId}`).emit('backrooms:player-left', { socketId: m.id });
}
function _spawnBackroomsMimic(instanceId, io) {
    const room = _backrooms.get(instanceId);
    if (!room || room.size < 2)
        return;
    const seed = _backroomsMeta.get(instanceId)?.seed ?? 0;
    ...
    io.to(`backrooms:${instanceId}`).emit('backrooms:player-joined', {
    ...
    const m = {
        ...
        tick: setInterval(() => {
            ...
            io.to(`backrooms:${instanceId}`).emit('backrooms:player-moved', { socketId: m.id, x: m.x, y: 1.6, z: m.z, ry, fl: false });
            ...
            if (Date.now() - m.bornAt > 6000) {
                for (const p of r.values()) {
                    ...
                    p.x = nx;
                    p.z = nz;
                    io.to(p.socketId).emit('backrooms:event', { kind: 'mimic_kill', x: nx, z: nz });
                    io.to(`backrooms:${instanceId}`).emit('backrooms:event', { kind: 'mimic_reveal', x: m.x, z: m.z });
                    _despawnBackroomsMimic(instanceId, io);
                    return;
                }
            }
    ...
    for (const [spaceId, room] of _spaces) {
        if (room.has(sid)) {
            ...
            if (opp) {
                ...
                if (oppP) {
                    oppP.hp = SPACE_MAX_HP;
                    io.to(`space:${spaceId}`).emit('space:duel_end', { winnerName: oppP.name, loserName: meP?.name ?? '?', forfeit: true });
                }
            }
            _clearDuel(sid);
            _clearRps(sid);
...
function assertInviteAllowance(fromId, toId, context) {
    const now = Date.now();
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('player:report', async (data, cb) => {
            try {
                ...
                const report = await createReport(reporterProfileId, reporter.username, parsed.targetProfileId, reported.username, parsed.roomId, parsed.reason, parsed.details, evidence);
        ...
        socket.on('room:create', async (data, cb) => {
            try {
                ...
                if (hostInRoom)
                    await applyIncognito(hostInRoom, profileId, room, parsed.incognito);
                ...
                try {
                    room.skin = await resolveRoomSkin(profileId);
                }
                catch { /* non-fatal */ }
                socket.join(room.id);
            ...
            try {
                ...
                if (!isRejoin && room.phase !== 'lobby') {
                    ...
                    if (parsed.joinMode === 'next_round' || parsed.joinMode === 'player') {
                        ...
                        try {
                            ...
                            if (!hidden)
                                broadcastSystemMsg(io, room, `${shown(player)} joined as spectator.`);
                        }
                        broadcastRoom(io, room);
                ...
                if (grace && player.isHost && (profileId === grace.profileId || (!profileId && player.name === grace.hostName))) {
                    ...
                    try {
                        ...
                        if (style) {
                            io.to(room.id).emit('room:entrance', {
        ...
        socket.on('room:kick', async ({ playerId }, cb) => {
            try {
                ...
                if (!actor.isHost && !isMod)
                    throw new Error('Only the host or a moderator can kick players.');
                if (room.phase !== 'lobby' && !isMod)
                ...
                if (room.phase !== 'lobby' && targetSock) {
                    ...
                    handlePlayerLeave(io, targetSock, room.id, target.id);
                }
                else {
        ...
        socket.on('room:warn', async ({ playerId }, cb) => {
            try {
                ...
                if (!actor.isHost && !isMod)
                    throw new Error('Only the host or a moderator can warn players.');
                const target = room.players.get(playerId);
                if (!target)
                    throw new Error('Player not found.');
                if (target.id === actor.id)
        ...
        socket.on('room:set_incognito', async ({ on }, cb) => {
            try {
                const room = getRoomFromSocket(socket);
                const player = getPlayerOrError(socket, room);
                if (room.phase !== 'lobby')
                    throw new Error('შენიღბვის შეცვლა მხოლოდ ლობიშია შესაძლებელი.');
                if (on) {
                    if (!(await limitsFor(socket.data.profileId)).incognito)
                        throw new Error('ინკოგნიტო ვერიფიცირებულებისთვისაა.');
                    player.incognito = true;
                    player.anonAlias = aliasFor(`${player.id}:${room.id}`);
                }
                else {
        ...
        socket.on('room:to-spectator', async (cb) => {
            try {
                const room = getRoomFromSocket(socket);
                const player = getPlayerOrError(socket, room);
                if (room.phase !== 'lobby' && room.phase !== 'game_over')
                    throw new Error('You can switch to spectator only in the lobby.');
                if (player.isSpectator)
                    throw new Error('Already a spectator.');
                ...
                if (!hidden)
                    broadcastSystemMsg(io, room, `${shown(player)} switched to spectators.`);
                broadcastRoom(io, room);
                cb?.(ok(null));
            }
            catch (e) {
        ...
        socket.on('room:to-player', (cb) => {
            try {
                const room = getRoomFromSocket(socket);
                const player = getPlayerOrError(socket, room);
                if (!player.isSpectator)
                    throw new Error('Already a player.');
                if (room.phase !== 'lobby' && room.phase !== 'game_over') {
                    if (player.isQueuedNextRound)
                        throw new Error('Already in the next-round queue.');
                    const position = enqueueForNextRound(room, player.id);
                    broadcastSystemMsg(io, room, `${shown(player)} joined the queue for next round (#${position}).`);
                    broadcastQueueUpdated(io, room);
                    broadcastRoom(io, room);
        ...
        socket.on('game:sheriff_check', async ({ targetId }, cb) => {
            try {
                const room = getRoomFromSocket(socket);
                const actor = getPlayerOrError(socket, room);
            ...
            try {
                ...
                if (profileId) {
                    ...
                    if (verdict.flag) {
                        ...
                        const recentChat = room.chat.slice(-20).map(m => ({
                        ...
                        createReport('system', 'ავტომატური ფილტრი', profileId, me?.username ?? player.name, room.id, 'other', verdict.reason, recentChat, verdict.flag).then(() => notifyMods(io, 'new_report', `ავტო-flag: ${me?.username ?? player.name} — ${verdict.reason}`, me?.username ?? player.name))
                            .catch(() => { });
                        if (verdict.block)
                            throw new Error('შეტყობინება დაიბლოკა ავტომატური ფილტრით.');
                    }
                }
        ...
        socket.on('mod:announce', async ({ message, style }, cb) => {
            try {
                const modProfileId = socket.data.profileId;
                const mod = modProfileId ? await getPlayer(modProfileId) : null;
                if (!mod || !canDo(mod, 'ban_short'))
        ...
        socket.on('room:invite', async ({ friendProfileId }, cb) => {
            try {
                ...
                const friendSock = findSocketByProfile(io, targetId);
                if (friendSock) {
        ...
        socket.on('mars:status', async (cb) => {
            try {
        ...
        socket.on('mars:directory', async (data, cb) => {
            try {
                cb(ok(await marsDirectory(Number(data?.limit) || 20)));
            }
            catch (e) {
                cb(err(e.message));
            }
        ...
        socket.on('mars:lookup', async ({ code }, cb) => {
            try {
        ...
        socket.on('mars:memorial_save', async (data, cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId)
                    throw new Error('ACCESS DENIED — ჯერ შედი სისტემაში.');
                ...
                const subject = await marsUpsertMemorial(profileId, me?.username ?? '', data?.memorialId ? String(data.memorialId) : null, {
        ...
        socket.on('mars:my_memorials', async (cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId) {
                    cb(ok([]));
         ...
         */
        socket.on('mars:open', async ({ code }, cb) => {
            try {
                const s = await marsGetByCode(String(code ?? '').trim());
                if (!s) {
                    cb(ok(null));
                    ...
                    if (!isMod) {
                        cb(ok({
                            record: {
                                ...
                                stewardName: '', stewardRelation: '', sampleStatus: 'none',
        ...
        socket.on('mars:memory_delete', async ({ memoryId }, cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId)
         ...
         */
        socket.on('mars:speak', async ({ code, text }, cb) => {
            try {
        ...
        // ── Reports on records ─────────────────────────────────────────────
        socket.on('mars:report', async (data, cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId)
                    throw new Error('ACCESS DENIED — ჯერ შედი სისტემაში.');
        ...
        socket.on('mars:report_reasons', (cb) => { cb(ok(MARS_REPORT_REASONS)); });
        /** Moderation queue. */
        socket.on('mars:reports', async (data, cb) => {
            try {
                const profileId = socket.data.profileId;
                const me = profileId ? await getPlayer(profileId) : null;
                if (!me || !canDo(me, 'view_reports'))
                    throw new Error('Insufficient permissions.');
        ...
        socket.on('mars:report_resolve', async ({ reportId, action }, cb) => {
            try {
                const profileId = socket.data.profileId;
                const me = profileId ? await getPlayer(profileId) : null;
                if (!me || !canDo(me, 'resolve_reports'))
                    throw new Error('Insufficient permissions.');
                ...
                addModLog(act === 'remove' ? 'report_resolve' : 'report_reject', profileId, me.username, r.subjectId, '', null, `M.A.R.S. record ${r.subjectId} — ${act}`).catch(() => { });
                cb(ok(r));
            }
            catch (e) {
                cb(err(e.message));
        ...
        socket.on('mars:record_restore', async ({ subjectId }, cb) => {
            try {
                const profileId = socket.data.profileId;
                const me = profileId ? await getPlayer(profileId) : null;
                if (!me || !canDo(me, 'resolve_reports'))
                ...
                addModLog('report_reject', profileId, me.username, String(subjectId ?? ''), '', null, 'M.A.R.S. record restored').catch(() => { });
                cb(ok({ restored: true }));
            }
            catch (e) {
                cb(err(e.message));
        ...
        socket.on('mars:ask', async ({ text }, cb) => {
            try {
        ...
        socket.on('voice:camera', (payload) => {
            const roomId = socket.data.roomId;
            if (!roomId)
                return;
            socket.to(roomId).emit('voice:camera', { socketId: socket.id, on: !!payload?.on });
        });
        // ── Rematch ─────────────────────────────────────────────────────
        ...
        socket.on('perks:configure', async ({ which, mode }, cb) => {
            try {
                ...
                if (!profileId)
                    throw new Error('Not authenticated.');
                if (!TOGGLE_PERKS.includes(which))
        ...
        socket.on('perks:choose', async ({ which, value }, cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId)
                    throw new Error('Not authenticated.');
                if (!CHOICE_PERKS.includes(which))
                    throw new Error('უცნობი პარამეტრი');
         ...
         */
        socket.on('room:sticker', async ({ sticker }, cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId)
                    throw new Error('Not authenticated.');
                ...
                if (!player)
                    throw new Error('Player not found.');
                if (!STICKER_SET.includes(String(sticker)))
                    throw new Error('უცნობი სტიკერი');
                if (!stickerRateOk(socket.id))
                    throw new Error('ცოტა მოითმინე.');
                // Consume BEFORE broadcasting — if the spend fails there is nothing to show.
                if (!await consumeSticker(profileId))
                    throw new Error('სტიკერები აღარ გაქვს.');
                io.to(room.id).emit('room:sticker', {
        ...
        socket.on('perks:voicemask_get', async (cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId) {
                    cb(ok({ preset: null }));
        ...
        socket.on('community:boost_post', async ({ postId }, cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId)
                    throw new Error('Not authenticated.');
                ...
                if (!await consumePostBoost(profileId))
                    throw new Error('აწევა აღარ გაქვს.');
                let until;
                try {
        ...
        // ── ნუარი (adventure) ──────────────────────────────────────────────
        socket.on('noir:submit', async (data, cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId)
                    throw new Error('Not authenticated.');
        ...
        socket.on('noir:board', async (data, cb) => {
            try {
                cb(ok(await noirBoard(Number(data?.limit ?? 50))));
            }
            catch (e) {
                cb(err(e.message));
            }
        });
        socket.on('noir:me', async (cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId)
                    throw new Error('Not authenticated.');
        ...
        socket.on('appeal:create', async (data, cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId)
                    throw new Error('Not authenticated.');
                ...
                if (!ban && !mute)
                    throw new Error('შენ არ გაქვს გასაჩივრებელი შეზღუდვა.');
                const body = String(data?.body ?? '').trim();
                if (body.length < 10)
                    throw new Error('აღწერე მოკლედ რა მოხდა (მინიმუმ 10 სიმბოლო).');
        ...
        socket.on('appeal:mine', async (cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId) {
                    cb(ok({ restricted: null, appeal: null }));
                    return;
                }
                const ban = await getActiveBan(profileId);
                const mute = ban ? null : await getActiveMute(profileId);
        ...
        socket.on('appeal:list', async (data, cb) => {
            try {
                const me = socket.data.profileId ? await getPlayer(socket.data.profileId) : null;
                if (!me?.isModerator)
        ...
        socket.on('appeal:decide', async (data, cb) => {
            try {
                const me = socket.data.profileId ? await getPlayer(socket.data.profileId) : null;
                if (!me?.isModerator)
                    throw new Error('Moderators only.');
        ...
        socket.on('dm:image', async (data, cb) => {
            try {
                const senderId = socket.data.profileId;
                if (!senderId)
                    throw new Error('Not authenticated.');
        ...
        socket.on('dm:typing', async ({ conversationId }) => {
            try {
                ...
                const recipientSocket = findSocketByProfile(io, receiverId);
                if (recipientSocket)
                    recipientSocket.emit('dm:typing', { conversationId, fromUserId: senderId });
            }
            catch { /* best effort */ }
        });
                ...
                if (result.added && result.authorId && result.authorId !== profileId) {
                    (async () => {
                        try {
                            ...
                            const notif = await createNotification(result.authorId, 'post_reaction', `${emoji} რეაქცია`, `${reactor?.username ?? 'ვიღაცამ'} დაარეაქთა ${emoji} შენს პოსტზე`, null, { actorId: profileId, actorAvatarUrl: reactor?.avatarUrl ?? null, postId: postId });
            ...
            try {
                ...
                if (result.added && result.authorId && result.authorId !== profileId) {
                    (async () => {
                        try {
                            const reactor = await getPlayer(profileId);
                            const notif = await createNotification(result.authorId, 'comment_reaction', `${emoji} რეაქცია`, `${reactor?.username ?? 'ვიღაცამ'} დაარეაქთა ${emoji} შენს კომენტარზე`, null, { actorId: profileId, actorAvatarUrl: reactor?.avatarUrl ?? null });
                            const ownerSock = findSocketByProfile(io, result.authorId);
                            if (ownerSock)
                                ownerSock.emit('community:notification', notif);
                        }
        ...
        socket.on('community:follow', async ({ targetId }, cb) => {
            try {
                ...
                const notif = await createNotification(targetId, 'new_follower', '👤 ახალი მიმდევარი', `${follower?.username ?? 'ვიღაცამ'} დაგიწყო გამოწერა`, null, { actorId: profileId, actorAvatarUrl: follower?.avatarUrl ?? null });
                const targetSock = findSocketByProfile(io, targetId);
                if (targetSock)
        ...
        socket.on('community:post_get', async (data, cb) => {
            try {
                const viewerId = socket.data.profileId ?? '';
        ...
        socket.on('community:story_react', async ({ storyId, reaction }, cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId)
                    throw new Error('Not authenticated.');
                ...
                if (result.added && result.authorId && result.authorId !== profileId) {
                    const reactor = await getPlayer(profileId);
                    const notif = await createNotification(result.authorId, 'story_reaction', `${reaction} სტორის რეაქცია`, `${reactor?.username ?? 'ვიღაცამ'} დაარეაქთა ${reaction} შენს სტორიზე`, `story:${storyId}`, { actorId: profileId, actorAvatarUrl: reactor?.avatarUrl ?? null });
                    const ownerSock = findSocketByProfile(io, result.authorId);
                    if (ownerSock) {
                        ownerSock.emit('community:notification', notif);
        ...
        socket.on('community:story_reactions', async ({ storyId }, cb) => {
            try {
        ...
        socket.on('community:story_notif_unread', async (cb) => {
            try {
                const profileId = socket.data.profileId;
        ...
        socket.on('community:story_notif_read', async (cb) => {
            try {
                const profileId = socket.data.profileId;
                if (profileId)
                    await markStoryReactionNotificationsRead(profileId);
                cb?.(ok(null));
            }
            catch (e) {
                cb?.(err(e.message));
        ...
        socket.on('community:story_create', async ({ imageUrl, caption, tags, musicVideoId, musicTitle }, cb) => {
            try {
                const profileId = socket.data.profileId;
                if (!profileId)
                    throw new Error('Not authenticated.');
                ...
                if (tags && tags.length > 0) {
                    ...
                    for (const t of tags) {
                        ...
                        createNotification(t.id, 'story_tag', '📸 სთორიზე დაგთეგეს', `${myName}-მა სთორიზე დაგთეგა`, null, { actorId: profileId, actorAvatarUrl: me?.avatarUrl ?? null }).then(notif => {
                            const sock = findSocketByProfile(io, t.id);
                            if (sock)
        ...
        socket.on('live:start', async (data, cb) => {
            try {
                ...
                io.emit('live:started', { hostId: me, sessionId: session.id, title: session.title });
                cb(ok(session));
            }
            catch (e) {
        ...
        socket.on('live:end', async (payload, cb) => {
            ...
            try {
                ...
                if (summary) {
                    io.to(roomFor(summary.id)).emit('live:ended', { sessionId: summary.id });
                    io.emit('live:stopped', { hostId: me, sessionId: summary.id });
                }
                ack(ok(summary));
            }
            catch (e) {
        ...
        socket.on('live:leave', async (data, cb) => {
            try {
                ...
                io.to(roomFor(sessionId)).emit('live:viewers', { sessionId, viewers });
                if (typeof cb === 'function')
                    cb(ok(null));
            }
            catch {
        ...
        socket.on('live:heart', async (data) => {
            try {
                ...
                socket.to(roomFor(sessionId)).emit('live:hearted', { sessionId, userId: me });
            }
        ...
        socket.on('space:duel_challenge', ({ targetSocketId }, cb) => {
            try {
                ...
                if (_duelPending.has(targetSocketId))
                    return cb?.({ ok: false, error: 'Invite already pending.' });
        ...
        socket.on('space:duel_respond', ({ fromSocketId, accept }, cb) => {
            try {
                ...
                if (t) {
                    clearTimeout(t);
                    _duelPendingTimer.delete(socket.id);
                }
                const wasPending = _duelPending.get(socket.id) === fromSocketId;
                _duelPending.delete(socket.id);
                if (!me || !challenger || !wasPending)
                    return cb?.({ ok: false, error: 'Invite expired.' });
                if (!accept) {
        ...
        socket.on('space:rps_challenge', ({ targetSocketId }, cb) => {
            try {
                const spaceId = _spaceOfSocket(socket.id);
                if (!spaceId)
                    return cb?.({ ok: false });
                ...
                const me = room?.get(socket.id);
                const target = room?.get(targetSocketId);
                if (!me || !target || socket.id === targetSocketId)
                    return cb?.({ ok: false });
                if (_rpsPending.has(targetSocketId))
                    return cb?.({ ok: false, error: 'Invite already pending.' });
                const key = _rpsKey(socket.id, targetSocketId);
                if (_rpsGames.has(key))
                    return cb?.({ ok: false, error: 'Already playing.' });
        ...
        socket.on('space:rps_respond', ({ fromSocketId, accept }, cb) => {
            try {
                const spaceId = _spaceOfSocket(socket.id);
                if (!spaceId)
                    return cb?.({ ok: false });
                ...
                if (t) {
                    clearTimeout(t);
                    _rpsPendingTimer.delete(socket.id);
                }
                ...
                const game = { aSocketId: fromSocketId, aName: challenger.name, bSocketId: socket.id, bName: me.name, round: 1, aWins: 0, bWins: 0 };
        ...
        socket.on('space:rps_pick', ({ choice }, cb) => {
            try {
                ...
                let myKey = '';
                for (const [key, g] of _rpsGames) {
                    if (g.aSocketId === socket.id || g.bSocketId === socket.id) {
                        myGame = g;
                        myKey = key;
                        break;
                    }
                ...
                if (!myGame)
                    return cb?.({ ok: false, error: 'No active game.' });
                if (socket.id === myGame.aSocketId)
                    myGame.aPick = choice;
                else
                    myGame.bPick = choice;
                cb?.({ ok: true });
        ...
        // ── Truth or Dare ("სიმართლე თუ მოქმედება") ───────────────────────
        socket.on('space:tod_challenge', ({ targetSocketId }, cb) => {
            try {
                const spaceId = _spaceOfSocket(socket.id);
                if (!spaceId)
                    return cb?.({ ok: false });
                ...
                if (!me || !target || socket.id === targetSocketId)
                    return cb?.({ ok: false });
                if (_todPending.has(targetSocketId))
                    return cb?.({ ok: false, error: 'Invite already pending.' });
                _todPending.set(targetSocketId, socket.id);
                const timer = setTimeout(() => { if (_todPending.get(targetSocketId) === socket.id) {
                    _todPending.delete(targetSocketId);
                    _todPendingTimer.delete(targetSocketId);
                } }, 20000);
                _todPendingTimer.set(targetSocketId, timer);
                io.to(targetSocketId).emit('space:tod_invite', { fromSocketId: socket.id, fromName: me.name });
                cb?.({ ok: true });
            }
            catch {
                cb?.({ ok: false });
        ...
        socket.on('space:tod_respond', ({ fromSocketId, accept }, cb) => {
            try {
                const spaceId = _spaceOfSocket(socket.id);
                if (!spaceId)
                    return cb?.({ ok: false });
                ...
                if (t) {
                    clearTimeout(t);
                    _todPendingTimer.delete(socket.id);
                }
                ...
                if (!me || !challenger || !wasPending)
                    return cb?.({ ok: false, error: 'Invite expired.' });
                if (!accept) {
                    io.to(fromSocketId).emit('space:tod_declined', { byName: me.name });
                    return cb?.({ ok: true });
                }
                io.to(fromSocketId).emit('space:tod_start', { opponent: me.name, opponentSocketId: socket.id });
                io.to(socket.id).emit('space:tod_start', { opponent: challenger.name, opponentSocketId: fromSocketId });
                cb?.({ ok: true });
            }
            catch {
                cb?.({ ok: false });
        ...
        socket.on('space:tod_pick', ({ choice }, cb) => {
            try {
                ...
                io.to(`space:${spaceId}`).emit('space:tod_question', { playerName: me.name, choice, label, question });
                cb?.({ ok: true });
            }
            catch {
                cb?.({ ok: false });
        ...
        socket.on('space:reaction_start', (_, cb) => {
            try {
                const spaceId = _spaceOfSocket(socket.id);
                if (!spaceId)
                    return cb?.({ ok: false });
                if (_reactionGames.has(spaceId))
                    return cb?.({ ok: false, error: 'Already running.' });
                const room = _spaces.get(spaceId);
                const me = room?.get(socket.id);
                if (!me)
                    return cb?.({ ok: false });
                ...
                const game = { spaceId, phase: 'joining', starterSocketId: socket.id, players, results: [], timers: [] };
                _reactionGames.set(spaceId, game);
                // Broadcast invite to everyone in the space
                io.to(`space:${spaceId}`).emit('space:reaction_invite', { starterName: me.name });
                // Auto-start after 8 seconds
                const startTimer = setTimeout(() => {
                    const g = _reactionGames.get(spaceId);
                    if (!g || g.phase !== 'joining')
                        return;
                    if (g.players.size < 2) {
                        io.to(`space:${spaceId}`).emit('space:reaction_cancelled', {});
                        _clearReactionGame(spaceId);
                        return;
                    }
                    g.phase = 'countdown';
                    io.to(`space:${spaceId}`).emit('space:reaction_countdown', { count: 3, players: [...g.players.entries()].map(([sid, name]) => ({ socketId: sid, name })) });
                    const t2 = setTimeout(() => io.to(`space:${spaceId}`).emit('space:reaction_countdown', { count: 2 }), 1000);
                    const t3 = setTimeout(() => io.to(`space:${spaceId}`).emit('space:reaction_countdown', { count: 1 }), 2000);
                    ...
                    const tGo = setTimeout(() => {
                        ...
                        gg.phase = 'go';
                        gg.goTime = Date.now();
                        io.to(`space:${spaceId}`).emit('space:reaction_go', {});
        ...
        socket.on('space:reaction_join', (_, cb) => {
            try {
                const spaceId = _spaceOfSocket(socket.id);
                if (!spaceId)
                    return cb?.({ ok: false });
                ...
                io.to(`space:${spaceId}`).emit('space:reaction_player_joined', { name: me.name, count: g.players.size });
                cb?.({ ok: true });
            }
            catch {
                cb?.({ ok: false });
        ...
        socket.on('space:reaction_tap', (_, cb) => {
            try {
                const spaceId = _spaceOfSocket(socket.id);
                if (!spaceId)
                    return cb?.({ ok: false });
                const g = _reactionGames.get(spaceId);
                if (!g || g.phase !== 'go' || !g.goTime)
                    return cb?.({ ok: false });
                if (!g.players.has(socket.id))
                    return cb?.({ ok: false });
                if (g.results.some(r => r.socketId === socket.id))
                    return cb?.({ ok: false }); // already tapped
                ...
                if (g.results.length >= g.players.size)
                    _finishReaction(spaceId, io);
            }
            catch {
                cb?.({ ok: false });
        ...
        socket.on('space:reaction_leaderboard', (_, cb) => {
            try {
                const spaceId = _spaceOfSocket(socket.id);
                if (!spaceId)
                    return cb?.({ ok: false });
        ...
        socket.on('backrooms:join', ({ instanceId, name, skin, shirt }, cb) => {
            try {
                ...
                const room = _backrooms.get(id) ?? new Map();
                if (!_backrooms.has(id))
                    _backrooms.set(id, room);
                if (!room.has(socket.id) && room.size >= meta.maxPlayers) {
        ...
        socket.on('backrooms:move', ({ x, y, z, ry, fl }) => {
            ...
            for (const [instanceId, room] of _backrooms) {
                ...
                p.fl = !!fl;
                socket.to(`backrooms:${instanceId}`).emit('backrooms:player-moved', {
        ...
        socket.on('world:chat', ({ text }) => {
            ...
            for (const [worldId, room] of _worlds) {
                if (room.has(socket.id)) {
                    ...
                    io.to(`world:${worldId}`).emit('world:chat', { socketId: socket.id, name, text: msg, at: Date.now() });
                    return;
                }
            }
        });
        ...
        socket.on('space:yt-search', async ({ query }, cb) => {
        ...
        socket.on('space:voice-ghost_join', async (cb) => {
            try {
                const pid = socket.data.profileId;
                const mod = pid ? await getPlayer(pid) : null;
                if (!mod || mod.moderatorLevel !== 'owner' || !isGhost(pid))
                ...
                if (!spaceId)
                    return cb?.({ ok: false, error: 'Not observing a space.' });
                const voices = _spaceVoice.get(spaceId);
        ...
        socket.on('disconnect', () => {
            ...
            /*
             * Closing the tab is how most people stop watching, and a viewer count
             * that only ever goes up is worse than none. The host's own session is
             * NOT ended here — a phone that loses signal for ten seconds is still
             * live, and the heartbeat is what decides that.
             */
            {
                const gone = socket.data.profileId;
                if (gone) {
                    for (const sessionId of forgetViewer(gone)) {
                        io.to(roomFor(sessionId)).emit('live:viewers', { sessionId, viewers: 0 });
                    }
                }
            }
