...
import { createClan, getClan, getClanByPlayer, getClanMembershipByPlayer, getAllClans, getClanMembers, joinClan, leaveClan, setClanImage, } from './services/clanService.js';
...
export function attachSocketHandlers(io) {
    io.on('connection', (socket) => {
        ...
        // ── Economy — Coins & Gifts ─────────────────────────────────────
        // ── Lobby Chat ───────────────────────────────────────────────────
        socket.on('lobby:history', async (data, cb) => {
        ...
        socket.on('lobby:delete_msg', async ({ msgId }, cb) => {
            try {
                ...
                const profile = await getPlayer(profileId);
                if (!profile?.isModerator) {
                    cb(err('Not authorized.'));
                    return;
                }
                const idx = lobbyChatHistory.findIndex(m => m.id === msgId);
                if (idx !== -1)
                    lobbyChatHistory.splice(idx, 1);
