...
function coinsForProduct(productId) {
...
const CLIENT_BUILD = '2026-07-28-v580';
...
app.post('/api/iap/revenuecat', express.json(), async (req, res) => {
    if (!REVENUECAT_WEBHOOK_AUTH) {
        res.status(503).json({ ok: false, error: 'IAP not configured.' });
        return;
    }
    ...
    const transactionId = event.transaction_id ?? event.store_transaction_id ?? event.id ?? '';
    const coins = coinsForProduct(productId);
    if (!profileId || !productId || !transactionId) {
        res.status(400).json({ ok: false, error: 'Missing app_user_id/product_id/transaction_id.' });
        return;
    }
    if (coins == null) {
        console.warn(`[iap] unknown product_id: ${productId}`);
        res.json({ ok: true, ignored: 'unknown_product' });
        return;
    }
    try {
...
async function tryInitDb(attempt = 1) {
    try {
        ...
        const runLeague = () => import('./services/clanLeagueService.js')
            .then(m => m.settleLeague((pid, amt, desc) => grantCoins('system', pid, amt, desc).then(() => { }), (pid, title, body) => import('./services/communityService.js')
            .then(c => c.createNotification(pid, 'leaderboard_reward', title, body, null))
            .then(() => { })))
            .then(rs => { for (const r of rs)
            console.log(`[ClanLeague] week ${new Date(r.weekStart).toISOString()}: ${r.paidClans} clan(s), ${r.paidPlayers} player(s)`); })
