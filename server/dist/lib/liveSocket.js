 ...
 */
export function socketForUser(io, userId, storedSocketId) {
    const direct = io.sockets.sockets.get(storedSocketId);
    if (direct && direct.connected)
        return direct;
    ...
    if (!userId || userId === storedSocketId)
        return null;
    for (const s of io.sockets.sockets.values()) {
        if (s.connected && s.data?.profileId === userId)
            return s;
    }
    return null;
}
 ...
 */
export function emitToPlayers(io, players, event, payload, rebind) {
