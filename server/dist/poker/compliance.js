 ...
 */
import { CURRENT_CAPABILITIES } from '../future-economy/EconomyProvider.js';
export const DEFAULT_COMPLIANCE = {
    productDescriptor: 'social poker',
    noticeShort: 'უფასო სოციალური პოკერი — ჩიპებს არ აქვს ფულადი ღირებულება.',
    noticeLong: 'ეს არის უფასო, სოციალური პოკერი. მაგიდაზე გამოყენებული ჩიპები არის მხოლოდ '
...
let active = { ...DEFAULT_COMPLIANCE };
export function getCompliance() { return active; }
/** Admin/config surface: the text is editable, the guarantees below are not. */
export function setCompliance(patch) {
    active = { ...active, ...patch };
    return active;
}
export function complianceFacts(capabilities = CURRENT_CAPABILITIES) {
    return {
 ...
 */
export function assertSocialOnly(capabilities = CURRENT_CAPABILITIES) {
    const forbidden = ['transfer', 'redeem', 'deposit', 'withdrawal']
        .filter(key => capabilities[key]);
    if (forbidden.length > 0) {
        throw new Error(`Poker refuses to start: the product ships as ${active.productDescriptor} with a `
