...
function swallow(where) {
    return err => {
        failures += 1;
        console.warn(`[poker/db] ${where} failed:`, err?.message ?? err);
    };
}
 ...
 * order flexible.
 */
export async function initializePokerSchema() {
    await sql `
 ...
 */
export function recordHand(history) {
    void writeHand(history).catch(swallow('recordHand'));
}
async function writeHand(h) {
...
export async function getStats(playerId) {
    const rows = await sql `SELECT * FROM poker_stats WHERE player_id = ${playerId}`;
    const row = rows[0];
...
export async function getRecentHands(playerId, limit = 25) {
...
export const LEADERBOARD_METRICS = ['hands_won', 'win_rate', 'biggest_pot'];
 ...
 */
export async function rebuildLeaderboards(now = Date.now()) {
    try {
...
export async function getLeaderboard(metric, limit = 50) {
    const rows = await sql `
...
// ─── Retention ───────────────────────────────────────────────────────────────
export const RETENTION_DAYS = { hands: 90, audit: 180, sessions: 90 };
/**
 ...
 */
export async function pruneOldRecords(now = Date.now()) {
    const day = 86400000;
    try {
        ...
        swallow('pruneOldRecords')(e);
    }
}
/** Data-deletion request: forget who, keep what happened. */
export async function anonymisePlayer(playerId) {
    try {
