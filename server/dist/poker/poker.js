...
let limiter = null;
let sinks = {};
export function setPokerSinks(next) { sinks = next; }
function ensure(io) {
    if (service)
        return service;
    limiter = new RateLimiter();
    service = new PokerTableService({
        emit: (event) => deliver(io, event),
 ...
 */
function deliver(io, event) {
    if (event.playerIds.length === 0) {
        if (event.tableId)
            io.to(room(event.tableId)).emit(event.event, event.payload);
        else
    ...
    for (const playerId of event.playerIds) {
        for (const socketId of sockets.get(playerId) ?? []) {
            io.sockets.sockets.get(socketId)?.emit(event.event, event.payload);
        }
    }
}
...
const int = (value, fallback) => typeof value === 'number' && Number.isFinite(value) ? Math.floor(value) : fallback;
function identity(socket, name) {
...
// ─── Handlers ────────────────────────────────────────────────────────────────
export function registerPokerHandlers(io, socket) {
    if (!pokerEnabled())
        return;
    const svc = ensure(io);
    /**
     ...
     */
    const handle = (action, fn) => (data, cb) => {
        ...
        try {
            ...
            if (e instanceof TableError)
                ...
                console.error('[poker]', e);
                reply(err('INTERNAL'));
            }
        }
    };
    const join = (tableId) => { socket.join(room(tableId)); };
    socket.on('poker:list', handle('list', () => ({
        tables: svc.listTables(),
        compliance: { notice: getCompliance(), facts: complianceFacts() },
    })));
    socket.on('poker:create', handle('create', (me, d) => {
     ...
     */
    socket.on('poker:action', handle('action', (me, d) => {
        const type = text(d.type, 8);
        if (!['fold', 'check', 'call', 'raise', 'allIn'].includes(type))
            throw new TableError('BAD_ACTION', 'Unknown action.');
        svc.act(text(d.tableId, 64), me.playerId, {
            handId: text(d.handId, 64),
            actionSeq: int(d.actionSeq, -1),
            action: { type: type, amount: Math.max(0, int(d.amount, 0)) },
        });
        return { ok: true };
    }));
    /**
     ...
     */
    socket.on('poker:add_bot', async (d, cb) => {
        const reply = typeof cb === 'function' ? cb : () => { };
        try {
            const me = identity(socket);
            if (!me)
                return reply(err('AUTH_REQUIRED'));
            if (!(await isOwner(me.playerId)))
                return reply(err('OWNER_ONLY'));
            const tableId = text(d?.tableId, 64);
            const table = svc.getTable(tableId);
            if (!table)
                return reply(err('NO_TABLE'));
            const free = Array.from({ length: table.config.maxSeats }, (_, i) => i)
                .find(i => !table.seats.some(s => s.seat === i));
            if (free === undefined)
                return reply(err('TABLE_FULL'));
            const bot = {
                playerId: newBotId(),
            ...
            if (e instanceof TableError)
                ...
                console.error('[poker/bot]', e);
                reply(err('INTERNAL'));
            }
        }
    });
    socket.on('poker:clear_bots', async (d, cb) => {
        const reply = typeof cb === 'function' ? cb : () => { };
        try {
            const me = identity(socket);
            if (!me)
    ...
    socket.on('poker:chat', handle('chat', (me, d) => {
        const tableId = text(d.tableId, 64);
        const table = svc.getTable(tableId);
        ...
        if (!seated && !table.observers.has(me.playerId))
            throw new TableError('NOT_AT_TABLE', 'You are not at this table.');
        const body = text(d.text, 200);
        if (!body)
            throw new TableError('EMPTY', 'Nothing to say.');
        io.to(room(tableId)).emit('poker:chat', {
     ...
     */
    socket.on('poker:resume', handle('resume', me => {
    ...
    if (me) {
        track(me.playerId, socket.id);
        svc.setConnected(me.playerId, true);
    }
}
function track(playerId, socketId) {
 ...
 */
export function handlePokerDisconnect(_io, socket) {
    const profileId = socket.data.profileId;
...
export function pokerService() { return service; }
