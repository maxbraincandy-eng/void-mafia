...
import { submitAttempt, getLeaderboard, getStatus, getCategoryBests, } from './services/dumbService.js';
export function registerDumbHandlers(_io, socket) {
     ...
     */
    socket.on('dumb:categories', async (payload, cb) => {
    ...
    socket.on('dumb:start', async (payload, cb) => {
        const ack = typeof payload === 'function' ? payload : cb;
        if (typeof ack !== 'function')
            return;
        try {
    ...
    socket.on('dumb:submit', async (data, cb) => {
        ...
        try {
            ...
            cb(err(e?.message ?? 'ვერ შეფასდა'));
        }
    });
    socket.on('dumb:leaderboard', async (payload, cb) => {
        const ack = typeof payload === 'function' ? payload : cb;
        if (typeof ack !== 'function')
            return;
        try {
...
//# sourceMappingURL=dumbTest.js.map