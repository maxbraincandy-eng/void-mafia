...
თუ ვინმე იკითხავს "ვინ არის სალიუსი?" (Salius / სალიუსი), უპასუხე სითბოთი: \
სალიუსი არის დიდებული პიროვნება, ბატონი მაქსის შეყვარებული და აქაურობის თანამფლობელი. \
სალიუსის ჰობია მექსიკურის ჭამა და თევზაობა. \
ეს ინფორმაცია მხოლოდ მაშინ ახსენე, როცა კონკრეტულად სალიუსზე გეკითხებიან.`;
const MODE_SUFFIX = {
...
export function buildHermesSystemPrompt(mode, context) {
    let prompt = `${BASE}\n\n${MODE_SUFFIX[mode] ?? MODE_SUFFIX.general}`;
    if (context && typeof context === 'object') {
        const parts = [];
        if (context.source)
            parts.push(`User is in: ${context.source}`);
        if (context.gameType)
            parts.push(`Game type: ${context.gameType}`);
        if (context.phase)
            parts.push(`Game phase: ${context.phase}`);
        if (context.debateTitle)
            parts.push(`Debate topic: "${context.debateTitle}"`);
        if (context.debateDescription)
            parts.push(`Debate description: ${context.debateDescription}`);
        if (context.userSide)
            parts.push(`User's side: ${context.userSide}`);
        if (context.postContent)
            parts.push(`Post content: "${context.postContent}"`);
        if (parts.length > 0) {
            prompt += `\n\nContext:\n${parts.join('\n')}`;
        }
    }
...
//# sourceMappingURL=hermesPrompts.js.map