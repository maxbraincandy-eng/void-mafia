 ...
 */
export const CURRENT_CAPABILITIES: EconomyCapabilities = {
  grant: true,
  spend: true,
  transfer: false,
  redeem: false,
...
export class EconomyDisabledError extends Error {
  constructor(capability: keyof EconomyCapabilities) {
    super(
      `Economy capability "${capability}" is disabled in this product. ` +
 ...
 */
export class DisabledEconomyProvider implements EconomyProvider {
  constructor(private readonly capabilities: EconomyCapabilities = CURRENT_CAPABILITIES) {}

  async getBalance(): Promise<number> { return 0; }

  async addGameplayCurrency(_playerId: string, amount: number): Promise<number> {
    if (!this.capabilities.grant) throw new EconomyDisabledError('grant');
    return amount;
  }

  async spendGameplayCurrency(_playerId: string, amount: number): Promise<number> {
    if (!this.capabilities.spend) throw new EconomyDisabledError('spend');
    return amount;
  }

  async transferCurrency(): Promise<never> { throw new EconomyDisabledError('transfer'); }
  async redeemCurrency(): Promise<never> { throw new EconomyDisabledError('redeem'); }

  cashValue(): CashValue { return null; }
}
