...
export function createHermesRouter(): Router {
  ...
  router.get('/probe-groq', async (_req, res) => {
    try {
      if (!process.env.GROQ_API_KEY) { res.json({ ok: false, error: 'GROQ_API_KEY not set' }); return; }
      const { GroqProvider } = await import('../ai/groqProvider.js');
      const p = new GroqProvider();
      let catalogue: string[] = [];
      let listError: string | null = null;
      try { catalogue = await p.listUsable(); }
      catch (e: any) { listError = `${e?.status ?? ''} ${e?.message ?? e}`.trim(); }
      ...
      try {
        ...
        reply = r.text;
      } catch (e: any) { chatError = (e?.message ?? String(e)).slice(0, 200); }
