...
function pick<T>(list: T[], salt: number): T | null {
  if (list.length === 0) return null;
  return list[Math.abs(salt) % list.length] ?? null;
}

const aliveBots = (m: XmMatch): XmSeat[] =>
  m.seats.filter(s => isBot(s.userId) && s.alive && !s.left);

const isMafiaRole = (seat: XmSeat) => seat.role === 'mafia' || seat.role === 'don';
 ...
 */
export function tick(matchId: string): boolean {
  const m = getMatch(matchId);
  if (!m || m.dissolved || m.phase === 'finished') return false;
  ...
  if (m.phase === 'speech' && !m.introRound) {
    ...
    if (!speakerId || !isBot(speakerId)) return false;
    if (m.nominatedBy[speakerId] !== undefined) return false;
    // Not every speech ends in a nomination, or every day would have a vote.
    if ((salt + m.speechIdx) % 3 !== 0) return false;
  ...
  if (m.phase === 'vote') {
    ...
    if (!waiting || m.nominations.length === 0) return false;
...
export function hasBots(matchId: string): boolean {
  const m = getMatch(matchId);
