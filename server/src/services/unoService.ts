...
export interface UnoMatch {
  ...
  hostId: string;
  players: UnoPlayer[];
  spectatorIds: string[]; // socketIds of spectators
  settings: UnoSettings;
  deck: UnoCard[];
  discardPile: UnoCard[];
  hands: Record<string, UnoCard[]>; // userId → cards
...
// Safe public state sent to clients
export interface UnoPublicState {
  ...
  voiceSessionId: string;
  chat: UnoChatMsg[];
  myHand: UnoCard[]; // only for the requesting player
  myUserId: string;
}
...
const matches = new Map<string, UnoMatch>();
...
export function createMatch(hostId: string, hostSocketId: string, nickname: string, opts: Partial<UnoSettings> = {}): UnoMatch {
  const id = randomBytes(8).toString('hex');
  const settings: UnoSettings = {
    maxPlayers: 4,
...
export function getMatch(matchId: string): UnoMatch | null {
  return matches.get(matchId) ?? null;
}

export function getMatchByCode(code: string): UnoMatch | null {
...
export function getMatchForSocket(socketId: string): UnoMatch | null {
  for (const m of matches.values()) {
    if (m.players.some(p => p.socketId === socketId)) return m;
    if (m.spectatorIds.includes(socketId)) return m;
  }
  return null;
}
...
export function joinMatch(
  ...
  socketId: string,
  nickname: string,
): { match: UnoMatch; isNew: boolean } | null {
  ...
  if (m.players.length >= m.settings.maxPlayers) return null;
...
export function spectateMatch(matchId: string, socketId: string): UnoMatch | null {
  const m = matches.get(matchId);
...
export function leaveMatch(matchId: string, userId: string, socketId: string): UnoMatch | null {
  const m = matches.get(matchId);
  if (!m) return null;
...
export function startMatch(matchId: string, hostId: string): UnoMatch | null {
  const m = matches.get(matchId);
  ...
  for (let i = 0; i < 7; i++) {
    for (const player of m.players) {
      ...
      if (card) m.hands[player.userId]!.push(card);
    }
  }
  ...
  if (!startCard) {
    ...
    m.currentColor = startCard.color as GameColor;
  }
...
function drawCards(m: UnoMatch, userId: string, count: number): void {
...
// ── Helper: advance turn ──────────────────────────────────────────────────────
function advanceTurn(m: UnoMatch, skipCount = 1): void {
...
function isPlayable(card: UnoCard, currentColor: GameColor, topDiscard: UnoCard, pendingDrawType: 'draw2' | 'wild4' | null, pendingDrawCount: number, stackingEnabled: boolean): boolean {
  // If there's a pending draw stack
  if (pendingDrawCount > 0 && pendingDrawType) {
    if (!stackingEnabled) return false; // can't play any card when stacking disabled
...
export function playCard(
...
): { match: UnoMatch; error?: string } | null {
  const m = matches.get(matchId);
  if (!m) return null;
  if (m.status !== 'active') return { match: m, error: 'Game not active' };
  ...
  const hand = m.hands[userId];
  if (!hand) return { match: m, error: 'No hand found' };

  const cardIdx = hand.findIndex(c => c.id === cardId);
  if (cardIdx === -1) return { match: m, error: 'Card not in hand' };

  const card = hand[cardIdx]!;
  const topDiscard = m.discardPile[m.discardPile.length - 1];
  const currentColor = m.currentColor ?? 'red';

  if (!topDiscard) return { match: m, error: 'No top discard' };
  ...
  if (card.type !== 'wild' && card.type !== 'wild4') {
    m.currentColor = card.color as GameColor;
  }
...
// ── Draw card(s) ──────────────────────────────────────────────────────────────
export function drawCard(matchId: string, userId: string): { match: UnoMatch; drawnCard?: UnoCard; error?: string } | null {
  const m = matches.get(matchId);
  if (!m) return null;
  if (m.status !== 'active') return { match: m, error: 'Game not active' };
...
// ── Call UNO ──────────────────────────────────────────────────────────────────
export function callUno(matchId: string, userId: string): UnoMatch | null {
  const m = matches.get(matchId);
...
export function applyUnoPenaltyIfNeeded(m: UnoMatch): void {
  if (!m.settings.unoPenaltyEnabled) return;
...
// ── Send chat ─────────────────────────────────────────────────────────────────
export function sendChat(matchId: string, userId: string, nickname: string, text: string): { match: UnoMatch; msg: UnoChatMsg } | null {
  const m = matches.get(matchId);
  if (!m) return null;

  const msg: UnoChatMsg = { userId, nickname, text: text.trim().slice(0, 300), ts: Date.now() };
  m.chat.push(msg);
...
// ── Rematch ───────────────────────────────────────────────────────────────────
export function rematch(matchId: string, hostId: string): UnoMatch | null {
  const m = matches.get(matchId);
  if (!m || m.hostId !== hostId || m.status !== 'finished') return null;
...
export function getSafeState(m: UnoMatch, viewerUserId: string): UnoPublicState {
  const currentPlayer = m.players[m.currentPlayerIndex];
