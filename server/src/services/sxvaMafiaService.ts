...
export interface XmMatch {
  ...
  settings: {
    speechSeconds: number; nightSeconds: number;
    /** Seconds per CANDIDATE, not per vote — see the note above startVote. */
    voteSeconds: number;
    lastWordsSeconds: number; floorControl: boolean;
  };
  /**
 ...
 */
export function mafiaRoomFor(matchId: string): string { return `sxvamafia_${matchId}:mafia`; }
 ...
 */
export function mafiaChannelOpen(m: XmMatch): boolean {
  if (m.phase === 'mafia_meet' || m.phase === 'plan_night') return true;
  return m.phase === 'night' && !m.sport;
}
 ...
 */
export function mafiaChannelRole(m: XmMatch, userId: string): MafiaChannelRole {
  if (!mafiaChannelOpen(m)) return null;
  if (m.hostId === userId) return 'listen';
  const seat = findByUser(m, userId);
  if (!seat || !seat.alive || !isMafiaRole(seat.role)) return null;
  return 'speak';
}
...
export function setSettings(matchId: string, byUserId: string, patch: Partial<XmMatch['settings']> & { sport?: boolean }): XmMatch | null {
  ...
  if (m.phase === 'lobby') {
    ...
    if (patch.voteSeconds != null) m.settings.voteSeconds = Math.max(3, Math.min(30, Math.floor(patch.voteSeconds)));
...
function shootable(m: XmMatch, userId: string | null): XmSeat | null {
...
function resolveKill(m: XmMatch): XmSeat | null {
   ...
   */
  if (m.night.hostShot !== undefined) return shootable(m, m.night.hostShot);
 ...
 */
export function setHostShot(matchId: string, byUserId: string, targetUserId: string | null): XmMatch | null {
  const m = matches.get(matchId);
  ...
  if (targetUserId !== null) {
    ...
    if (!target || !target.alive || isMafiaRole(target.role)) return null;
  }
 ...
 */
function nightNeedsHost(m: XmMatch): boolean {
  if (m.sport) return false;                       // blind by design; see setHostShot
  if (m.night.hostShot !== undefined) return false; // already ruled
  const mafia = aliveMafia(m);
  if (mafia.length === 0) return false;             // nobody left to shoot
 ...
 */
const HOST_SHOT_GRACE_MS = 90_000;
...
function maybeAutoNight(m: XmMatch): void {
  if (m.phase !== 'night') return;
  if (!nightAllActed(m)) return;
  if (!nightNeedsHost(m)) { resolveNight(m); return; }
  // Everyone has acted and the shot is split: hand it to the host, on a clock.
  if (!m.nightEndsAt) m.nightEndsAt = Date.now() + HOST_SHOT_GRACE_MS;
}
 ...
 * closing the book.
 */
function candidateWindow(m: XmMatch): number {
 ...
 */
function stepCandidate(m: XmMatch): void {
 ...
 */
export function advanceCandidateAuto(matchId: string): XmMatch | null {
  const m = matches.get(matchId);
  if (!m || m.phase !== 'vote') return null;
  stepCandidate(m);
  return m;
