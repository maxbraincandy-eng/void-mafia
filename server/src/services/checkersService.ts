...
export type MoveResult =
  | {
      ok: true;
      board: CheckersBoard;
      captured: { row: number; col: number } | null;
      promoted: boolean;
      mustContinueFrom: { row: number; col: number } | null;
      winnerColor: PieceColor | null;
    }
...
// ── Board Logic ────────────────────────────────────────────────────────
export function initBoard(): CheckersBoard {
  const board: CheckersBoard = Array.from({ length: 8 }, () => Array(8).fill(null) as (CheckersPiece | null)[]);
  for (let r = 0; r < 8; r++) {
    for (let c = 0; c < 8; c++) {
      if ((r + c) % 2 !== 1) continue;
...
export function applyMove(
  ...
  if (piece.color !== color) return { ok: false, error: 'Not your piece.' };
  ...
  // Deep-copy board
  const nb: CheckersBoard = board.map(row => row.map(cell => (cell ? { ...cell } : null)));

  nb[toRow][toCol] = { ...piece };
  nb[fromRow][fromCol] = null;

  let captured: { row: number; col: number } | null = null;
  if (move.capture) {
  ...
  if (!nb[toRow][toCol]!.king) {
    if (color === 'red' && toRow === 0) { nb[toRow][toCol]!.king = true; promoted = true; }
    if (color === 'black' && toRow === 7) { nb[toRow][toCol]!.king = true; promoted = true; }
  }

  // Multi-jump: allow another capture from the same piece (unless just promoted)
  let nextContinue: { row: number; col: number } | null = null;
...
export function createMatch(
  ...
  do { code = `CK-${Math.floor(1000 + Math.random() * 9000)}`; } while (usedCodes.has(code));
...
export function getMatch(id: string): CheckersMatch | undefined { return matches.get(id); }

export function getMatchByCode(code: string): CheckersMatch | undefined {
  const upper = code.trim().toUpperCase();
  for (const m of matches.values()) if (m.code === upper) return m;
  return undefined;
}

export function deleteMatch(id: string): void { matches.delete(id); }

export function getOpenMatches(): CheckersMatch[] {
