...
async function rankOf(correct: number, durationMs: number): Promise<number> {
  const [row] = await sql`
    WITH best AS (
...
export async function getLeaderboard(viewerId: string | null, limit = 50): Promise<DumbLeaderRow[]> {
...
export async function getStatus(userId: string): Promise<DumbStatus> {
  const [agg] = await sql`
