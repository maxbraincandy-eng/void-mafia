...
export interface SpyfallPublicState {
  ...
  hostId: string;
  maxPlayers: number;
  players: { userId: string; socketId: string; nickname: string; seat: number; connected: boolean; score: number; hasVoted: boolean }[];
  settings: { rounds: number; discussSeconds: number };
  round: number;
  endsAt: number;
...
export interface SpyfallListItem {
  id: string; code: string; hostName: string; playerCount: number; maxPlayers: number; status: SpyfallStatus;
}

const matches = new Map<string, SpyfallMatch>();
...
export function createMatch(hostId: string, socketId: string, nickname: string, opts: { maxPlayers?: number; rounds?: number; discussSeconds?: number }): SpyfallMatch {
...
export function getMatch(id: string): SpyfallMatch | null { return matches.get(id) ?? null; }
export function getMatchByCode(code: string): SpyfallMatch | null { for (const m of matches.values()) if (m.code === code && m.status !== 'finished') return m; return null; }
export function getMatchForSocket(socketId: string): SpyfallMatch | null { for (const m of matches.values()) if (m.players.some(p => p.socketId === socketId)) return m; return null; }
export function listMatches(): SpyfallListItem[] {
...
export function joinMatch(matchId: string, userId: string, socketId: string, nickname: string): { match: SpyfallMatch; isNew: boolean } | null {
...
export function leaveMatch(matchId: string, userId: string): SpyfallMatch | null {
  const m = matches.get(matchId);
  if (!m) return null;
...
export function resumeForUser(userId: string, socketId: string): SpyfallMatch | null {
...
export function dissolveMatch(matchId: string, leaverId: string): SpyfallMatch | null {
  const m = matches.get(matchId);
  if (!m) return null;
...
function setupRound(m: SpyfallMatch): void {
  ...
  if (pool.length === 0) { m.usedLocationIdxs = []; pool = SPYFALL_LOCATIONS.map((_, i) => i); }
...
export function startMatch(matchId: string, byUserId: string): SpyfallMatch | null {
  const m = matches.get(matchId);
...
function finishRound(m: SpyfallMatch, outcome: SpyfallOutcome, extra: { accusedName?: string | null; guessedLocation?: string | null }): void {
  const spy = m.players.find(p => p.isSpy)!;
  const loc = SPYFALL_LOCATIONS[m.locationIdx!]!;
  if (outcome === 'spy_caught' || outcome === 'spy_wrong') {
    for (const p of m.players) if (!p.isSpy) p.score += 1;
...
function maybeTally(m: SpyfallMatch): void {
  if (m.status !== 'voting') return;
  const voters = m.players.filter(p => p.connected);
  if (voters.length === 0 || voters.some(p => !p.vote)) return;
  const counts = new Map<string, number>();
  for (const p of m.players) if (p.vote) counts.set(p.vote, (counts.get(p.vote) ?? 0) + 1);
  ...
  if (tie || !best) { finishRound(m, 'spy_escaped', {}); return; }
  const accused = m.players.find(p => p.userId === best)!;
  if (accused.isSpy) finishRound(m, 'spy_caught', { accusedName: accused.nickname });
  else finishRound(m, 'wrong_accused', { accusedName: accused.nickname });
}

export function castVote(matchId: string, byUserId: string, targetId: string): SpyfallMatch | null {
  const m = matches.get(matchId);
  ...
  if (!voter || !target || voter.userId === target.userId) return null;
  voter.vote = targetId;
  maybeTally(m);
  return m;
}
...
export function nextRound(matchId: string, byUserId: string): SpyfallMatch | null {
  const m = matches.get(matchId);
...
export function rematch(matchId: string, byUserId: string): SpyfallMatch | null {
  const m = matches.get(matchId);
...
export function getSafeState(m: SpyfallMatch, viewerUserId: string): SpyfallPublicState {
