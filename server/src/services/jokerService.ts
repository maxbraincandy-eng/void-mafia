...
export interface JokerPlayer {
  ...
  avatarUrl?: string | null; // uploaded picture, when they have one
}

export interface JokerSettings {
  mode: 'classic' | 'nines_only';
  bonusEnabled: boolean;         // premium (პრემია) on/off
 ...
 */
const PLANS: Record<'classic' | 'nines_only', number[][]> = {
  classic: [
    [1, 2, 3, 4, 5, 6, 7, 8, 9],
    [9, 9, 9, 9],
    [9, 8, 7, 6, 5, 4, 3, 2, 1],
...
export function resolveTrick(
  ...
  for (const played of contenders) {
    if (played.card.rank > winner.card.rank) winner = played;
  }
 ...
 */
export function forbiddenBid(match: JokerMatch): number | null {
  const declared = match.players.filter(p => match.declarations[p.id] !== null);
  if (declared.length !== match.players.length - 1) return null;
 ...
 */
export function bidTension(match: JokerMatch): { sum: number; diff: number; kind: 'tear' | 'stuff' | 'even' } {
  const cardCount = match.roundPlan[match.currentRoundIndex] ?? 0;
...
export function calcRoundScore(
  ...
  cardCount: number,
  khishtiPenalty = 0,
): { points: number; khishti: boolean; exact: boolean } {
 ...
 */
function settlePremium(match: JokerMatch, pulkaId: number): {
  ...
  if (rounds.length === 0) return { bonus, penalty };

  // Best DEAL of the column — a fall is never anybody's best, so the floor is 0.
  const best = (id: string) => Math.max(0, ...rounds.map(r => r.points[id] ?? 0));
  const cleanSweep = (id: string) =>
    rounds.every(r => (r.taken[id] ?? 0) === (r.declarations[id] ?? 0));

  const winners = match.players.filter(p => cleanSweep(p.id));
  if (winners.length === 0) return { bonus, penalty };

  for (const w of winners) {
    const b = best(w.id);
    if (b > 0) { bonus[w.id] = b; match.scores[w.id] = (match.scores[w.id] ?? 0) + b; }
  }
  for (const other of match.players) {
    if (winners.some(w => w.id === other.id)) continue;
    const lost = best(other.id);
    if (lost > 0) { penalty[other.id] = lost; match.scores[other.id] = (match.scores[other.id] ?? 0) - lost; }
  }
