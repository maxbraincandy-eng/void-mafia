...
import {
  BY_LEVEL, getQuestion, LEVEL_WEIGHT,
  type LogicLevel, type LogicQuestion,
} from '../data/logic/index.js';

/** Paper composition: 25 questions, weighted toward the middle. */
export const EXAM_PLAN: Array<[LogicLevel, number]> = [
...
export interface ExamStatus {
  ...
  best: { score: number; correct: number; total: number; at: number } | null;
  attempts: number;
  totalQuestions: number;
  examMs: number;
}

export async function examStatus(userId: string): Promise<ExamStatus> {
...
// ── sitting the paper ─────────────────────────────────────────────────
export async function startExam(userId: string): Promise<{ view: any } | { error: string }> {
  ...
  for (const s of sessions.values()) {
    if (s.userId === userId && !s.finished && Date.now() < s.endsAt) return { view: viewOf(s) };
  }

  const picks: LogicQuestion[] = [];
  for (const [lv, n] of EXAM_PLAN) picks.push(...shuffle(BY_LEVEL[lv]).slice(0, n));
  const now = Date.now();
  const s: ExamSession = {
...
export function getExam(id: string): ExamSession | null { return sessions.get(id) ?? null; }
export function examView(s: ExamSession) { return viewOf(s); }

/** Record an answer. No feedback comes back — this is an exam. */
export function answerExam(examId: string, userId: string, chosen: number): { done: boolean; next: any } | null {
  const s = sessions.get(examId);
  if (!s || s.userId !== userId || s.finished) return null;
  if (Date.now() > s.endsAt + LATE_GRACE_MS) return null;          // clock ran out
  const cur = s.questions[s.idx];
  if (!cur) return null;
...
export interface ExamResult {
  ...
  best: boolean;                 // did this become their best sitting
  coins: number;
...
function gradeOf(score: number): string {
  if (score >= 90) return 'უმაღლესი — ლოგიკის ოსტატი';
  if (score >= 80) return 'ძალიან კარგი';
  if (score >= 70) return 'კარგი';
  if (score >= 60) return 'დამაკმაყოფილებელი';
  if (score >= 50) return 'საზღვარზე';
  return 'გასამეორებელია';
}

export async function finishExam(examId: string, userId: string): Promise<ExamResult | null> {
  const s = sessions.get(examId);
  if (!s || s.userId !== userId || s.finished) return null;
  s.finished = true;
  ...
  for (const a of s.questions) {
    ...
    const b = (byLevel[q.level] ??= { correct: 0, total: 0 });
    b.total++;
    if (a.chosen !== undefined) answered++;
    if (a.correct) { earned += LEVEL_WEIGHT[q.level]; correct++; b.correct++; }
  }
  ...
  try { await grantCoins(userId, userId, coins, 'ლოგიკის გამოცდა'); } catch { /* wallet is best-effort */ }
...
// ── the exam's own leaderboard ────────────────────────────────────────
export interface ExamBoardRow {
  rank: number; userId: string; username: string; avatar: string; avatarUrl: string | null;
  country: string | null; score: number; correct: number; total: number; durationMs: number; at: number;
}

export async function examLeaderboard(scope: 'all' | 'week' | 'country', userId: string, limit = 50): Promise<ExamBoardRow[]> {
