...
export interface DrawPlayer {
  userId: string; socketId: string; nickname: string; seat: number;
  connected: boolean; score: number; guessedThisTurn: boolean; roundScore: number;
}
...
export interface DrawMatch {
  ...
  deck: string[]; deckPos: number;
  turnOrder: string[]; turnIdx: number; round: number;
...
export interface DrawPublicState {
  ...
  wordMask: string | null; // e.g. "_ _ _ _ (4)"
  revealedWord: string | null; // shown to all at turn end
  endsAt: number;
  iGuessed: boolean;
...
const matches = new Map<string, DrawMatch>();
...
export function createMatch(hostId: string, socketId: string, nickname: string, opts: { maxPlayers?: number; rounds?: number; drawSeconds?: number }): DrawMatch {
...
export function getMatch(id: string): DrawMatch | null { return matches.get(id) ?? null; }
...
export function leaveMatch(matchId: string, userId: string): DrawMatch | null {
  const m = matches.get(matchId);
  if (!m) return null;
...
export function resumeForUser(userId: string, socketId: string): DrawMatch | null {
...
export function disconnectSocket(socketId: string): string | null {
  ...
  p.connected = false;
  if (m.drawerId === p.userId) m.endsAt = Date.now();
  return m.id;
}

function threeWords(m: DrawMatch): string[] {
  const out: string[] = [];
  for (let i = 0; i < 3; i++) { if (m.deckPos >= m.deck.length) { m.deck = shuffle(DRAW_WORDS); m.deckPos = 0; } out.push(m.deck[m.deckPos++]!); }
  return out;
}

export function startMatch(matchId: string, byUserId: string): DrawMatch | null {
  const m = matches.get(matchId);
  if (!m || m.hostId !== byUserId || m.status !== 'waiting' || m.players.length < 2) return null;
  m.turnOrder = shuffle(m.players.map(p => p.userId));
  m.turnIdx = 0; m.round = 1;
...
export function beginChoosing(m: DrawMatch): void {
  ...
  m.endsAt = Date.now() + 15_000; // auto-pick fallback
}

export function chooseWord(matchId: string, byUserId: string, word: string): DrawMatch | null {
  const m = matches.get(matchId);
  if (!m || m.status !== 'choosing' || m.drawerId !== byUserId) return null;
...
export function autoChoose(matchId: string): DrawMatch | null {
  const m = matches.get(matchId);
  if (!m || m.status !== 'choosing') return null;
  return chooseWord(matchId, m.drawerId!, m.wordChoices[0]!);
}

export type GuessOut =
  | { kind: 'correct'; nickname: string; allGuessed: boolean }
  | { kind: 'chat'; nickname: string; text: string }
  | null;

export function guess(matchId: string, userId: string, text: string): GuessOut {
  const m = matches.get(matchId);
  if (!m || m.status !== 'drawing') return null;
  const p = m.players.find(pl => pl.userId === userId);
  if (!p) return null;
  const clean = String(text ?? '').trim().slice(0, 60);
  if (!clean) return null;
  // Drawer / already-guessed players just chat (relayed, not scored).
  if (userId === m.drawerId || p.guessedThisTurn) return { kind: 'chat', nickname: p.nickname, text: clean };
  if (m.word && norm(clean) === norm(m.word)) {
    ...
    if (drawer) drawer.score += 35; // drawer rewarded per correct guess
...
export function endTurn(matchId: string): DrawMatch | null {
  const m = matches.get(matchId);
  if (!m || (m.status !== 'drawing' && m.status !== 'choosing')) return null;
...
export function nextTurn(matchId: string): DrawMatch | null {
  const m = matches.get(matchId);
  if (!m || m.status !== 'turnend') return null;
  m.turnIdx++;
  // A full cycle through turnOrder = one round.
  if (m.turnIdx % m.turnOrder.length === 0) m.round++;
  if (m.round > m.settings.rounds) {
    m.status = 'finished';
    let best: DrawPlayer | null = null;
    for (const p of m.players) if (!best || p.score > best.score) best = p;
...
export function rematch(matchId: string, byUserId: string): DrawMatch | null {
  const m = matches.get(matchId);
...
export function addSeg(matchId: string, byUserId: string, seg: DrawSeg): boolean {
  const m = matches.get(matchId);
  if (!m || m.status !== 'drawing' || m.drawerId !== byUserId) return false;
  if (m.segs.length < 6000) m.segs.push(seg);
  return true;
}
export function clearCanvas(matchId: string, byUserId: string): boolean {
  const m = matches.get(matchId);
  if (!m || m.status !== 'drawing' || m.drawerId !== byUserId) return false;
  m.segs = [];
  return true;
...
function maskFor(word: string | null): string | null {
  if (!word) return null;
  return word.split('').map(ch => (ch === ' ' ? '  ' : '_')).join(' ') + ` (${word.replace(/\s/g, '').length})`;
}

export function getSafeState(m: DrawMatch, viewerUserId: string): DrawPublicState {
