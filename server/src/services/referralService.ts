...
export async function applyReferral(referredId: string, referralCode: string): Promise<boolean> {
  if (!referralCode || !referredId) return false;
  try {
    // Check if this player was already referred
    const [existing] = await sql`SELECT id FROM referrals WHERE referred_id = ${referredId}` as any[];
...
export async function getReferralCount(playerId: string): Promise<number> {
  const [row] = await sql`SELECT COUNT(*) as cnt FROM referrals WHERE referrer_id = ${playerId}` as any[];
