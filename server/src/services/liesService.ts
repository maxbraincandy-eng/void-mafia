 ...
 */
export function resumeForUser(userId: string, socketId: string): LiesMatch | null {
...
function setupRound(m: LiesMatch): void {
  ...
  for (const p of m.players) { p.bluff = null; p.guessId = null; rejectedBluff.delete(p.userId); }
  m.status = 'writing';
