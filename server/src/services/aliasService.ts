...
export interface AliasPublicState {
  ...
  hostId: string;
  maxPlayers: number;
  players: { userId: string; nickname: string; seat: number; connected: boolean; team: 0 | 1 }[];
  settings: { targetScore: number; roundSeconds: number };
  scores: [number, number];
  activeTeam: 0 | 1;
...
export interface AliasListItem {
  id: string; code: string; hostName: string; playerCount: number; maxPlayers: number; status: AliasStatus;
}
...
export function createMatch(hostId: string, socketId: string, nickname: string, opts: { maxPlayers?: number; targetScore?: number; roundSeconds?: number }): AliasMatch {
...
export function getMatch(id: string): AliasMatch | null { return matches.get(id) ?? null; }
export function getMatchByCode(code: string): AliasMatch | null { for (const m of matches.values()) if (m.code === code && m.status !== 'finished') return m; return null; }
export function getMatchForSocket(socketId: string): AliasMatch | null { for (const m of matches.values()) if (m.players.some(p => p.socketId === socketId)) return m; return null; }
export function listMatches(): AliasListItem[] {
...
export function joinMatch(matchId: string, userId: string, socketId: string, nickname: string): { match: AliasMatch; isNew: boolean } | null {
...
export function switchTeam(matchId: string, userId: string): AliasMatch | null {
  const m = matches.get(matchId);
...
export function leaveMatch(matchId: string, userId: string): AliasMatch | null {
  const m = matches.get(matchId);
  if (!m) return null;
  ...
  if (m.turn && m.turn.describerId === userId) m.turn.endsAt = Date.now();
  return m;
}

/** Re-attach a player who came back on a new socket. See liesService.resumeForUser. */
export function resumeForUser(userId: string, socketId: string): AliasMatch | null {
...
function drawWord(m: AliasMatch): string {
  if (m.deckPos >= m.deck.length) { m.deck = shuffle(ALIAS_WORDS); m.deckPos = 0; }
  return m.deck[m.deckPos++]!;
}

export function startMatch(matchId: string, byUserId: string): AliasMatch | null {
  const m = matches.get(matchId);
  if (!m || m.hostId !== byUserId || m.status !== 'waiting') return null;
  if (m.players.filter(p => p.team === 0).length < 1 || m.players.filter(p => p.team === 1).length < 1) return null;
  if (m.players.length < 2) return null;
  m.status = 'play';
  m.scores = [0, 0];
  m.round = 1;
  m.activeTeam = 0;
...
export function startTurn(matchId: string, byUserId: string): AliasMatch | null {
  const m = matches.get(matchId);
  if (!m || m.status !== 'play' || m.turn) return null;
  ...
  if (describer.userId !== byUserId) return null;
...
export function markWord(matchId: string, byUserId: string, got: boolean): AliasMatch | null {
  const m = matches.get(matchId);
  if (!m || m.status !== 'play' || !m.turn || m.turn.describerId !== byUserId) return null;
  if (m.turn.endsAt <= Date.now()) return null;
  const word = m.currentWord ?? '';
  m.turn.log.push({ word, got });
  if (got) { m.turn.correct++; m.scores[m.turn.team]++; }
  else { m.turn.skipped++; m.scores[m.turn.team] = Math.max(0, m.scores[m.turn.team] - 1); }
...
export function endTurn(matchId: string): AliasMatch | null {
  const m = matches.get(matchId);
  if (!m || m.status !== 'play' || !m.turn) return null;
  m.describerIdx[m.turn.team]++;
  (m as any)._lastTurnLog = m.turn.log;
  m.turn = null;
  m.currentWord = null;
...
export function rematch(matchId: string, byUserId: string): AliasMatch | null {
  const m = matches.get(matchId);
...
export function getSafeState(m: AliasMatch, viewerUserId: string): AliasPublicState {
  ...
  let nextDescriberId: string | null = null;
  if (m.status === 'play' && !m.turn) {
    const tp = m.players.filter(p => p.team === m.activeTeam && p.connected);
    if (tp.length) nextDescriberId = tp[m.describerIdx[m.activeTeam] % tp.length]!.userId;
  }
  return {
    ...
    nextDescriberId,
    lastTurnLog: (m as any)._lastTurnLog ?? null,
