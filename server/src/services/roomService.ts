import {
  Room, Player, GameSettings, RoomPublic, PlayerPublic, Phase, RoomListItem, SpectatorQueueSettings,
} from '../types/index.js';
...
export function enqueueForNextRound(room: Room, playerId: string): number {
  ...
  // Check capacity: active players + queue must not exceed 16
...
export function removePlayer(room: Room, playerId: string): void {
  // Check if player is in the next-round queue first
...
export function rematchRoom(room: Room): void {
  ...
  // Promote queued players to lobby
  const promoted = promoteQueuedPlayers(room);
  // promoted players are already moved into room.players via their seat assignments;
  // but they still live in the players map — promoteQueuedPlayers mutates the Player
  // objects already in room.players (they were added as spectators earlier)
