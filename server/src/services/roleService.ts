...
export function buildRoleDeck(settings: GameSettings, playerCount: number): RoleKey[] {
  ...
  for (let i = 0; i < r.don; i++) deck.push('don');

  // Add town special roles
  for (let i = 0; i < r.sheriff; i++) deck.push('sheriff');
  for (let i = 0; i < r.doctor; i++) deck.push('doctor');

  // Add neutral roles
  for (let i = 0; i < r.maniac; i++) deck.push('maniac');
  for (let i = 0; i < r.jester; i++) deck.push('jester');
