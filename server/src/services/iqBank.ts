...
export interface IQShape {
  ...
  sides?: number;   // poly
  rot?: number;     // degrees (poly/arrow/flag)
  fill?: boolean;   // poly/circle/dots/arrow
  size?: number;    // 0..1 relative
  mirror?: boolean; // arrow/flag
  n?: number;       // dots / grid filled count
  top?: boolean; bottom?: boolean; left?: boolean; right?: boolean; // bars
}
export type IQCell = { shapes: IQShape[] } | { empty: true };

export type IQVisual =
  | { type: 'sequence'; cells: IQCell[] }          // last cell is { empty:true }
  | { type: 'matrix'; cols: number; cells: IQCell[] } // last cell is { empty:true }
  | { type: 'analogy'; a: IQCell; b: IQCell; c: IQCell }
  | { type: 'group'; cells: IQCell[] };            // odd-one-out
...
export interface IQQuestion {
  id: string;
  domain: IQDomain;
  difficulty: number; // 1..5, doubles as scoring weight
  prompt?: string;    // Georgian text / numeric sequence, optional for pure-visual
  visual?: IQVisual;
  options: IQOption[];
  correctId: string;  // SERVER ONLY — never sent to the client
}
...
const EMPTY: IQCell = { empty: true };
const P  = (sides: number, rot = 0, fill = false, size = 0.9): IQCell => ({ shapes: [{ t: 'poly', sides, rot, fill, size }] });
const DOT = (n: number, fill = true): IQCell => ({ shapes: [{ t: 'dots', n, fill }] });
const ARR = (rot: number, fill = false, mirror = false): IQCell => ({ shapes: [{ t: 'arrow', rot, fill, mirror }] });
const FLAG = (rot: number, mirror = false): IQCell => ({ shapes: [{ t: 'flag', rot, mirror }] });
const GRID = (n: number): IQCell => ({ shapes: [{ t: 'grid', n }] });
const BARS = (o: Partial<IQShape>): IQCell => ({ shapes: [{ t: 'bars', ...o }] });
const CIRC = (fill = false, size = 0.9): IQCell => ({ shapes: [{ t: 'circle', fill, size }] });
const vopt = (id: string, cell: IQCell): IQOption => ({ id, cell });
const topt = (id: string, text: string): IQOption => ({ id, text });

// ── The pool ────────────────────────────────────────────────────────────────
// Section order = presentation order; within a section, easy → hard.
export const IQ_POOL: IQQuestion[] = [
...
export const IQ_SECTION_ORDER: IQDomain[] = ['pattern', 'matrix', 'numeric', 'logic', 'spatial', 'verbal'];

export const IQ_DOMAIN_META: Record<IQDomain, { ka: string; key: string }> = {
...
export function assembleTest(): IQSafeQuestion[] {
  const ordered = IQ_SECTION_ORDER.flatMap(d =>
    IQ_POOL.filter(q => q.domain === d).sort((a, b) => a.difficulty - b.difficulty),
  );
...
export function getQuestion(id: string): IQQuestion | undefined { return BY_ID.get(id); }
export function totalQuestions(): number { return IQ_POOL.length; }
export function maxWeight(): number { return IQ_POOL.reduce((s, q) => s + q.difficulty, 0); }
