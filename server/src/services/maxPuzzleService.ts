...
export const MP_BOARD_SCOPES: MPBoardScope[] = ['independence', 'rationality', 'ambition', 'skepticism', 'risk', 'conformity'];

const SCOPE_COLUMN: Record<MPBoardScope, string> = {
...
export async function saveResult(userId: string, p: MPSubmitPayload): Promise<void> {
...
function mapRow(r: any, rank: number, col: string): MPBoardRow {
  return {
    rank,
    userId: r.user_id,
    username: r.username ?? '?',
 ...
 */
export async function getBoard(scope: MPBoardScope, viewerId: string | null, limit = 100): Promise<{ rows: MPBoardRow[]; myRow: MPBoardRow | null }> {
  ...
  if (!myRow && viewerId) {
    ...
    if (idx >= 0) myRow = mapRow(raw[idx], idx + 1, col);
  }
  return { rows, myRow };
}

export async function getMine(userId: string): Promise<MPBoardRow | null> {
  const [r] = await sql`
    SELECT r.*, p.username, p.avatar, p.avatar_url
    FROM max_puzzle_results r JOIN players p ON p.id = r.user_id
...
export async function modRemove(modId: string, targetUserId: string): Promise<{ removed: number }> {
  if (!(await isModerator(modId))) throw new Error('Not authorized');
  if (!targetUserId) throw new Error('No target');
  const rows = await sql`DELETE FROM max_puzzle_results WHERE user_id = ${targetUserId} RETURNING user_id` as any[];
