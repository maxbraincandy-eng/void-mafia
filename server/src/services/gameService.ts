...
export function setPhase(room: Room, phase: Phase): void {
  ...
  switch (phase) {
    ...
    case 'planning_night': {
      ...
      room.timer = dur;
      room.maxTimer = dur;
      if (room.donModeState) room.donModeState.planningNightCompleted = false;
      break;
    }
    case 'don_check': {
    ...
    case 'mafia_kill': {
      if (room.donModeState) room.donModeState.mafiaKillVotes = {};
      room.killedLastNight = [];
      room.savedLastNight = false;
      room.mafiaKillTarget = null;
      room.timer = room.settings.nightDuration;
    ...
    case 'revote': {
      room.votes = new Map();
      for (const p of room.players.values()) p.voteTarget = null;
      room.timer = room.settings.voteDuration;
      room.maxTimer = room.settings.voteDuration;
      break;
    }
    case 'double_elim_vote': {
      if (room.donModeState) room.donModeState.doubleEliminationVotes = {};
      for (const p of room.players.values()) p.hasActedThisPhase = false;
      room.timer = 30;
      room.maxTimer = 30;
      break;
    }
  ...
  switch (room.phase) {
    ...
    case 'voting': {
      ...
      if (room.deathSpeakerId) {
        ...
        if (testPlayer) testPlayer.isAlive = true;
        if (gameEnds) {
          if (testPlayer) { testPlayer.isAlive = false; testPlayer.deathType = 'vote'; }
    ...
    // ── Don Mode phases ──────────────────────────────────────────────
    case 'planning_night': {
      if (room.donModeState) room.donModeState.planningNightCompleted = true;
...
function detectDonModeTie(room: Room, candidateIds: string[]): string[] {
  const counts = new Map<string, number>();
  for (const [voterId, targetId] of room.votes.entries()) {
    if (!targetId) continue;
    if (candidateIds.length > 0 && !candidateIds.includes(targetId)) continue;
...
function resolveDonModeRevote(room: Room): 'winner' | 'tie' | 'no_elim' {
  if (!room.donModeState) return 'no_elim';
  ...
  const counts = new Map<string, number>();
  for (const [voterId, targetId] of room.votes.entries()) {
    if (!targetId || !candidates.includes(targetId)) continue;
    const voter = room.players.get(voterId);
    counts.set(targetId, (counts.get(targetId) ?? 0) + (voter?.role === 'mayor' ? 2 : 1));
  }
  if (counts.size === 0) return 'no_elim';
  const sorted = [...counts.entries()].sort((a, b) => b[1] - a[1]);
  const winnerId = sorted[0]![0];
  const winner = room.players.get(winnerId);
  if (!winner || !winner.isAlive) return 'no_elim';
...
function resolveDonModeKill(room: Room): void {
  if (!room.donModeState) return;
  ...
  if (!allVoted) { room.killedLastNight = []; room.mafiaKillTarget = null; return; }

  const choices = aliveMafia.map(p => votes[p.id]!);
  const allSame = choices.every(t => t === choices[0]);
  if (!allSame) { room.killedLastNight = []; room.mafiaKillTarget = null; return; }

  const targetId = choices[0]!;
  const target = room.players.get(targetId);
  if (!target || !target.isAlive) { room.killedLastNight = []; room.mafiaKillTarget = null; return; }
...
function enterDonNight(room: Room): Phase {
  const donAlive = [...room.players.values()].some(
    p => p.isAlive && !p.isSpectator && p.role === 'don',
  );
  if (donAlive) { setPhase(room, 'don_check'); return 'don_check'; }
  setPhase(room, 'mafia_kill');
  return 'mafia_kill';
}
...
function enterSheriffCheck(room: Room): Phase {
  const sheriffAlive = [...room.players.values()].some(
    p => p.isAlive && !p.isSpectator && p.role === 'sheriff',
  );
  if (sheriffAlive) { setPhase(room, 'sheriff_check'); return 'sheriff_check'; }
  return resolveDonNight(room);
}
...
function resolveDonNight(room: Room): Phase {
  ...
  if (room.killedLastNight.length > 0) {
    ...
    if (dying) { dying.isAlive = true; dying.deathType = null; }
    room.deathSpeakerId = primary.id;
    room.finalWordsReason = 'night_kill';
    setPhase(room, 'final_words');
    return 'final_words';
...
export function submitDonCheck(room: Room, actor: Player, targetId: string | null): void {
  if (room.phase !== 'don_check') throw new Error('Not Don Check phase.');
  if (actor.role !== 'don') throw new Error('Only the Don can perform this check.');
  if (!actor.isAlive) throw new Error('You are eliminated.');
  if (!room.donModeState) throw new Error('Don Mode is not active.');
  if (room.donModeState.donCheckDone) throw new Error('Don check already performed this night.');
  ...
  if (targetId) {
    const target = room.players.get(targetId);
    if (!target || !target.isAlive) throw new Error('Target not found or eliminated.');
    if (target.id === actor.id) throw new Error('Cannot check yourself.');
...
export function submitSheriffCheck(room: Room, actor: Player, targetId: string | null): void {
  if (room.phase !== 'sheriff_check') throw new Error('Not Sheriff Check phase.');
  if (actor.role !== 'sheriff') throw new Error('Only the Sheriff can perform this check.');
  if (!actor.isAlive) throw new Error('You are eliminated.');
  if (!room.donModeState) throw new Error('Don Mode is not active.');
  if (room.donModeState.sheriffCheckDone) throw new Error('Sheriff check already performed this night.');
...
export function submitMafiaKillVote(room: Room, actor: Player, targetId: string): void {
  if (room.phase !== 'mafia_kill') throw new Error('Not Mafia Kill phase.');
  if (actor.team !== 'mafia') throw new Error('Only Mafia team members can vote.');
  if (!actor.isAlive) throw new Error('You are eliminated.');
  if (!room.donModeState) throw new Error('Don Mode is not active.');

  const target = room.players.get(targetId);
  if (!target || !target.isAlive || target.isSpectator) throw new Error('Invalid target.');
  if (target.team === 'mafia') throw new Error('Cannot target a fellow Mafia member.');

  room.donModeState.mafiaKillVotes[actor.id] = targetId;
  actor.hasActedThisPhase = true;
}

export function submitDoubleEliminationVote(room: Room, voter: Player, yes: boolean): void {
...
export function submitNomination(room: Room, actor: Player, nomineeId: string | null): void {
  ...
  if (!room.settings.donMode && room.day < 2) throw new Error('Nominations are not allowed on Day 1.');
