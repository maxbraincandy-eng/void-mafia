...
export const SPORT_CITIZENS =
  SPORT_SEATS - SPORT_ROLES.don - SPORT_ROLES.mafia - SPORT_ROLES.sheriff;
 ...
 */
export function canStartSport(seatCount: number, donEnabled: boolean): { ok: boolean; reason: string | null } {
  if (!donEnabled) return { ok: false, reason: 'დონის კარტი ჩართული არ არის' };
 ...
 */
export function sheriffSees(role: string | null): boolean {
  return role === 'mafia';
}
 ...
 * back.
 */
export function agreedTarget(
  aliveTeam: { userId: string }[],
  picks: Record<string, string>,
): string | null {
  if (aliveTeam.length === 0) return null;
  let target: string | null = null;
  for (const member of aliveTeam) {
    const pick = picks[member.userId];
    if (!pick) return null;                 // somebody did not shoot
    if (target === null) target = pick;
    else if (target !== pick) return null;  // somebody shot somebody else
  }
  return target;
}
 ...
 */
export function tribunalElectorate<T extends { userId: string; alive: boolean }>(
  seats: T[],
  onTrial: string[],
): T[] {
 ...
 */
export function tribunalVerdict(punish: number, free: number): 'punish' | 'free' {
