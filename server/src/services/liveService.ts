 ...
 */
export async function payoutGifts(sessionId: string): Promise<{ hostId: string; coins: number } | null> {
...
export async function sendLiveGift(
  sessionId: string, senderId: string, giftId: string,
): Promise<SentLiveGift> {
  const gift = liveGift(giftId);
  if (!gift) throw new Error('ასეთი საჩუქარი არ არსებობს');

  const [session] = await sql`
    SELECT id, host_id FROM live_sessions WHERE id = ${sessionId} AND status = 'live'
  ` as any[];
  if (!session) throw new Error('ეთერი დასრულებულია');
   ...
   */
  if (hostId === senderId) throw new Error('საკუთარ ეთერს ვერ გაუგზავნი');
 ...
 */
export async function topGifters(sessionId: string, limit = 20): Promise<LiveGifter[]> {
  const rows = await sql`
