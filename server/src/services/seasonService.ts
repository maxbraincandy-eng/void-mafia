...
export async function getSeasonLeaderboard(seasonId: string, limit = 50): Promise<SeasonLeaderboardEntry[]> {
  const rows = await sql`
...
export async function getMySeasonHistory(profileId: string): Promise<SeasonResult[]> {
  const rows = await sql`
...
export async function processSeasonEnd(seasonId: string): Promise<void> {
  ...
  const [currentSeason] = await sql`SELECT number FROM seasons WHERE id = ${seasonId}` as any[];
