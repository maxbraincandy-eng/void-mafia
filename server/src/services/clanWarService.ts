...
async function rowToWar(r: any): Promise<ClanWar> {
  return {
    id: r.id,
    ...
    status: r.status as ClanWarStatus,
...
async function getWarWithNames(warId: string): Promise<ClanWar | null> {
...
export async function challengeClan(
  ...
  const [challenger] = await sql`SELECT id FROM clans WHERE id = ${challengerClanId}` as any[];
  if (!challenger) throw new Error('Your clan no longer exists.');

  const [defender] = await sql`SELECT id FROM clans WHERE id = ${defenderClanId}` as any[];
  ...
  const existing = await sql`
    ...
    LIMIT 1
  ` as any[];
  if (existing.length) throw new Error('There is already an active or pending war between these clans.');
...
export async function acceptWar(warId: string, defenderClanId: string): Promise<ClanWar> {
  const [row] = await sql`SELECT * FROM clan_wars WHERE id = ${warId}` as any[];
  if (!row) throw new Error('War not found.');
  if (row.defender_clan_id !== defenderClanId) throw new Error('Not authorised to accept this war.');
  if (row.status !== 'pending') throw new Error('War is no longer pending.');
...
export async function declineWar(warId: string, defenderClanId: string): Promise<ClanWar> {
  const [row] = await sql`SELECT * FROM clan_wars WHERE id = ${warId}` as any[];
  if (!row) throw new Error('War not found.');
  if (row.defender_clan_id !== defenderClanId) throw new Error('Not authorised to decline this war.');
  if (row.status !== 'pending') throw new Error('War is no longer pending.');
 ...
 */
export async function recordWarGame(
  gameId: string,
  winnerClanId: string,
  loserClanId: string,
  ...
  const [updated] = await sql`SELECT * FROM clan_wars WHERE id = ${war.id}` as any[];
...
export async function getActiveWar(clanId: string): Promise<ClanWar | null> {
  const rows = await sql`
    SELECT w.*,
           c1.name AS challenger_clan_name,
           c2.name AS defender_clan_name
...
export async function getWarHistory(clanId: string, limit = 10): Promise<ClanWar[]> {
  const rows = await sql`
    SELECT w.*,
           c1.name AS challenger_clan_name,
           c2.name AS defender_clan_name
