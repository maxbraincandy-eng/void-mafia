...
function rowToComment(r: any, reactions?: Record<string, number>, myReaction?: string | null): CommunityComment {
  return {
...
export async function getComments(postId: string, viewerId?: string): Promise<CommunityComment[]> {
  const rows = await sql`
  ...
  const reactionRows = commentIds.length > 0
    ? await sql`SELECT comment_id, emoji, COUNT(*) as cnt FROM community_comment_reactions WHERE comment_id = ANY(${commentIds}) GROUP BY comment_id, emoji` as any[]
    : [];
  const myReactionRows = commentIds.length > 0 && viewerId
    ? await sql`SELECT comment_id, emoji FROM community_comment_reactions WHERE comment_id = ANY(${commentIds}) AND player_id = ${viewerId}` as any[]
    : [];
  ...
  for (const r of myReactionRows) myReactionMap[r.comment_id] = r.emoji;
  return rows.map((r: any) => rowToComment(r, reactionsMap[r.id] ?? {}, myReactionMap[r.id] ?? null));
}

/** GIF comments carry a Tenor CDN url — never arbitrary/data URLs. */
function isValidCommentGif(url: string): boolean {
...
export async function addComment(
  ...
  if (parentId) {
    const [parent] = await sql`SELECT id, parent_id FROM community_post_comments WHERE id = ${parentId} AND post_id = ${postId}` as any[];
...
export async function toggleCommentLike(commentId: string, playerId: string): Promise<{ liked: boolean; likes: number }> {
  const [comment] = await sql`SELECT id FROM community_post_comments WHERE id = ${commentId}` as any[];
  if (!comment) throw new Error('Comment not found.');
  const [existing] = await sql`SELECT 1 as x FROM community_comment_likes WHERE comment_id = ${commentId} AND player_id = ${playerId}` as any[];
  if (existing) {
  ...
  const [row] = await sql`SELECT likes_count FROM community_post_comments WHERE id = ${commentId}` as any[];
  return { liked: !existing, likes: Number(row?.likes_count ?? 0) };
}

const VALID_REACTIONS = ['🔥','❤️','😂','💀','🤝','👑'];

export async function toggleCommentReaction(commentId: string, playerId: string, emoji: string): Promise<{ added: boolean; authorId: string; reactions: Record<string, number>; myReaction: string | null }> {
  if (!VALID_REACTIONS.includes(emoji)) throw new Error('Invalid reaction.');
  const [comment] = await sql`SELECT id, author_id FROM community_post_comments WHERE id = ${commentId}` as any[];
  if (!comment) throw new Error('Comment not found.');
  const [existing] = await sql`SELECT emoji FROM community_comment_reactions WHERE comment_id = ${commentId} AND player_id = ${playerId}` as any[];
  let added = false;
  ...
  const rows = await sql`SELECT emoji, COUNT(*) as cnt FROM community_comment_reactions WHERE comment_id = ${commentId} GROUP BY emoji` as any[];
...
export async function notifyMentions(
  text: string, actorId: string, actorName: string, context: 'post' | 'comment',
): Promise<string[]> {
  const usernames = [...new Set([...text.matchAll(/@([a-zA-Z0-9_.Ⴀ-ჿ-]{2,24})/g)].map(m => m[1]!))].slice(0, 8);
  if (!usernames.length) return [];
  const notified: string[] = [];
  for (const uname of usernames) {
    const [p] = await sql`SELECT id FROM players WHERE LOWER(username) = ${uname.toLowerCase()} LIMIT 1` as any[];
    if (!p || p.id === actorId) continue;
...
async function buildPostV2(row: any, viewerId: string): Promise<CommunityPostV2> {
  ...
  // Reactions
  const reactionRows = await sql<{ emoji: string; count: string }[]>`SELECT emoji, COUNT(*) as count FROM community_post_reactions WHERE post_id = ${row.id} GROUP BY emoji`;
  const reactions: Record<string, number> = {};
  reactionRows.forEach(r => { reactions[r.emoji] = Number(r.count); });
  let myReaction: string | null = null;
  if (viewerId) {
    const myR = await sql<{ emoji: string }[]>`SELECT emoji FROM community_post_reactions WHERE post_id = ${row.id} AND player_id = ${viewerId} LIMIT 1`;
 ...
 */
function parseImageUrls(raw: unknown, single: string | null): string[] {
...
export async function createPostV2(authorId: string, data: {
   ...
   */
  const MAX_IMAGES = 6;
  const MAX_IMAGE_BYTES = 680_000;
  const MAX_TOTAL_IMAGE_BYTES = 2_600_000;
  const images: string[] = (data.imageUrls ?? (data.imageUrl ? [data.imageUrl] : []))
  ...
  if (images.reduce((n: number, u: string) => n + u.length, 0) > MAX_TOTAL_IMAGE_BYTES) {
...
export async function editPost(postId: string, requesterId: string, newContent: string): Promise<CommunityPostV2> {
  ...
  if (!post) throw new Error('Post not found.');
  if (post.author_id !== requesterId) throw new Error('მხოლოდ ავტორს შეუძლია რედაქტირება.');
...
export async function deleteStory(id: string, requesterId: string, isMod: boolean): Promise<void> {
  const [row] = await sql`SELECT author_id FROM community_stories WHERE id = ${id}` as any[];
  if (!row) return;
...
export async function getPostById(postId: string, viewerId: string): Promise<CommunityPostV2 | null> {
  const rows = await sql<any[]>`
...
export async function togglePostReaction(postId: string, playerId: string, emoji: string): Promise<{ emoji: string | null; added: boolean; authorId: string; reactions: Record<string, number>; myReaction: string | null }> {
  if (!VALID_REACTIONS.includes(emoji)) throw new Error('Invalid reaction.');
  const [post] = await sql`SELECT author_id FROM community_posts WHERE id = ${postId}` as any[];
  if (!post) throw new Error('Post not found.');
...
export async function getWeeklyLeaderboard(startMs?: number, endMs?: number): Promise<Array<{ playerId: string; username: string; avatarUrl: string | null; score: number; rank: number }>> {
...
export async function settleWeeklyLeaderboard(
...
): Promise<{ paid: number; weekStart: number } | null> {
  const prevWeek = weekStartMs(Date.now()) - WEEK_MS; // the just-finished week
  const [already] = await sql`SELECT id FROM community_leaderboard_rewards WHERE week_start = ${prevWeek} LIMIT 1` as any[];
  if (already) return null; // already settled
  ...
  for (let i = 0; i < Math.min(3, leaders.length); i++) {
    ...
    await createNotification(leader.playerId, 'leaderboard_reward', '🏆 Weekly leaderboard reward',
      `You finished #${i + 1} last week and earned ${LEADERBOARD_PRIZES[i]} coins!`, null).catch(() => {});
    paid++;
  }
