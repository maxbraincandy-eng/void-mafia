...
export function registerWordHandlers(_io: AppServer, socket: AppSocket): void {
   ...
   */
  socket.on('word:state' as any, async (payload: any, cb: any) => {
    ...
    } catch (e: any) { ack(err(e?.message ?? 'ვერ ჩაიტვირთა.')); }
  });

  /** A guess. Rejections cost nothing — see submitGuess. */
  socket.on('word:guess' as any, async (payload: any, cb: any) => {
    const ack = typeof payload === 'function' ? payload : cb;
    if (typeof ack !== 'function') return;
    try {
  ...
  socket.on('word:board' as any, async (payload: any, cb: any) => {
    const ack = typeof payload === 'function' ? payload : cb;
    if (typeof ack !== 'function') return;
    try {
