...
import {
  ...
  type JokerMatch, type JokerPlayer, type JokerSettings, type Card,
...
export function registerJokerHandlers(io: AppServer, socket: AppSocket): void {
  ...
  // ── Play card ─────────────────────────────────────────────────────
  socket.on('joker:play-card' as any, (data: { matchId: string; card: Card }, cb: (res: any) => void) => {
    try {
      const match = getMatch(data.matchId);
      if (!match) return cb(err('Match not found.'));
      ...
      if (cardIndex === -1) return cb(err('Card not found in hand.'));
      ...
      if (match.currentTrick.length === 4) {
        ...
        setTimeout(() => {
          if (!getMatch(match.id)) return; // match was deleted
  ...
  socket.on('joker:rematch' as any, (data: { matchId: string }, cb: (res: any) => void) => {
    try {
      ...
      const creator: JokerPlayer = { ...old.players[0] };
