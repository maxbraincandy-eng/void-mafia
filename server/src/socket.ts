...
import {
  ...
  getFriendSuggestions, getPeopleToInvite,
...
import {
  addMedia as marsAddMedia, listMedia as marsListMedia, listMediaWithData as marsListMediaWithData,
  deleteMedia as marsDeleteMedia, updateMediaCaption as marsUpdateCaption, subjectIdOfMedia,
  addEvent as marsAddEvent, listEvents as marsListEvents, deleteEvent as marsDeleteEvent, subjectIdOfEvent,
  PHOTOS_MAX, VOICES_MAX, EVENTS_MAX,
} from './services/marsMedia.js';
 ...
 */
const marsMediaAt = new Map<string, number>();
function marsMediaRateOk(profileId: string): boolean {
  const now = Date.now();
...
function recoveryRateOk(socketId: string): boolean {
  const now = Date.now();
 ...
 */
const INVITABLE_GAMES: Record<string, { name: string; emoji: string }> = {
 ...
 */
const _invitesSent = new Map<string, number[]>();     // inviter → recent timestamps
...
export function attachSocketHandlers(io: AppServer): void {
  ...
  io.on('connection', (socket: AppSocket) => {
    ...
    socket.on('mod:close_room', async ({ roomId, reason }: { roomId: string; reason: string }, cb: any) => {
      try {
        ...
        if (isHostedMatch(roomId)) {
          ...
          if (!m) throw new Error('Room not found.');
      ...
      try {
        ...
        for (const h of listMatchesForMod()) {
          result.push({
            ...
            isPrivate: false,
            isPaused: false,
            players: h.players.map((p: { id: string; name: string; seat: number; isAlive: boolean; isConnected: boolean; profileId: string | null }) => ({
    ...
    socket.on('vip:perks' as any, async (cb: any) => {
      try {
    ...
    socket.on('vip:visitors' as any, async (cb: any) => {
      try {
        const me = socket.data.profileId;
        ...
        if (!limits.profileVisitors) return cb(ok({ locked: true, counts, visitors: [] }));
    ...
    const marsStewardOf = async (subjectId: string) => {
      const viewer = socket.data.profileId ?? null;
      if (!viewer) throw new Error('ACCESS DENIED — ჯერ შედი სისტემაში.');
      ...
      if (!isOwner) throw new Error('ამ ჩანაწერის რედაქტირების უფლება არ გაქვს.');
      return s;
    };

    socket.on('mars:media_add' as any, async (data: any, cb: any) => {
      try {
        const s = await marsStewardOf(data?.subjectId);
        if (!marsMediaRateOk(socket.data.profileId!)) throw new Error('THROTTLED — ცოტა მოითმინე.');
    ...
    socket.on('mars:media_update' as any, async (data: any, cb: any) => {
      try {
        const subjectId = await subjectIdOfMedia(String(data?.mediaId ?? ''));
        if (!subjectId) throw new Error('ვერ მოიძებნა.');
        await marsStewardOf(subjectId);
        cb(ok(await marsUpdateCaption(String(data.mediaId), String(data?.caption ?? ''), data?.year)));
      } catch (e: any) { cb(err(e.message)); }
    });

    socket.on('mars:media_delete' as any, async (data: any, cb: any) => {
      try {
        const subjectId = await subjectIdOfMedia(String(data?.mediaId ?? ''));
        if (!subjectId) { cb(ok({ deleted: false })); return; }
        await marsStewardOf(subjectId);
        cb(ok({ deleted: await marsDeleteMedia(String(data.mediaId)) }));
      } catch (e: any) { cb(err(e.message)); }
    });

    socket.on('mars:event_add' as any, async (data: any, cb: any) => {
      try {
        const s = await marsStewardOf(data?.subjectId);
    ...
    socket.on('mars:event_delete' as any, async (data: any, cb: any) => {
      try {
    ...
    socket.on('mars:media_limits' as any, (cb: any) => {
    ...
    socket.on('recovery:reset' as any, async (data: any, cb: any) => {
      try {
        ...
        if (!recoveryRateOk(socket.id)) throw new Error('ძალიან ბევრი მცდელობა. ცოტა მოითმინე.');
     ...
     */
    socket.on('invite:people' as any, async ({ q }: { q?: string }, cb: (r: any) => void) => {
      try {
        const profileId = socket.data.profileId;
        if (!profileId) throw new Error('Not authenticated.');
    ...
    socket.on('game:invite', async ({ targetProfileId, game, code }: { targetProfileId: string; game: string; code: string }, cb) => {
      try {
        ...
        if (!label || !code) throw new Error('Invalid invite.');
    ...
    socket.on('perks:get' as any, async (cb: any) => {
      try {
        const profileId = socket.data.profileId;
        if (!profileId) throw new Error('Not authenticated.');
    ...
    socket.on('perks:buy' as any, async ({ perkId }: { perkId: string }, cb: any) => {
      try {
        const profileId = socket.data.profileId;
        if (!profileId) throw new Error('Not authenticated.');
    ...
    socket.on('spectator:toggle_invisible' as any, async ({ on }: { on: boolean }, cb: any) => {
      try {
        const profileId = socket.data.profileId;
        if (!profileId) throw new Error('Not authenticated.');
        const room = getRoomFromSocket(socket);
        const player = room.players.get(socket.data.playerId ?? '');
        if (!player || !player.isSpectator) throw new Error('მხოლოდ დამკვირვებელს შეუძლია.');
    ...
    socket.on('dm:voice', async (data: { conversationId: string; audioData: string; duration: number }, cb: any) => {
      try {
        ...
        if (data.audioData.length > voiceCap) throw new Error('Voice message too large.');
        ...
        const claimedFx = String((data as any).fx ?? '');
