...
import { buildTableSummary, buildTableView, type TableView } from './views.js';
import { systemClock, type Clock, type TimerHandle } from './clock.js';
import {
  DEFAULT_TABLE_CONFIG,
  type AuditSink, type EventSink, type HandHistory, type HistorySink,
  type PlayerRef, type PokerTable, type Seat, type TableConfig, type TableSummary,
} from './types.js';
...
export class PokerTableService {
  ...
  private actionTimers = new Map<string, { handle: TimerHandle; deadline: number; playerId: string }>();
  /** At most one pending "deal the next hand" per table. */
  private startTimers = new Map<string, TimerHandle>();
  private otherTimers = new Map<string, TimerHandle[]>();

  private readonly clock: Clock;
  private readonly rng: Randomness;
  ...
  getTable(tableId: string): PokerTable | null { return this.tables.get(tableId) ?? null; }
  ...
  createTable(host: PlayerRef, patch: Partial<TableConfig> & { name?: string }): PokerTable {
    ...
    for (let tries = 0; this.byCode.has(code) && tries < 20; tries++) code = this.newCode();
    if (this.byCode.has(code)) throw new TableError('CODE_COLLISION', 'Could not allocate a table code.');
  ...
  private validateConfig(config: TableConfig): TableConfig {
  ...
  sit(tableId: string, player: PlayerRef, seatNo: number): Seat {
    ...
    if (table.status === 'closed') throw new TableError('CLOSED', 'That table has closed.');
    ...
    if (table.seats.some(s => s.seat === seatNo)) throw new TableError('SEAT_TAKEN', 'That seat is taken.');
  ...
  leave(tableId: string, playerId: string): void {
    ...
    if (playerId === table.hostId) {
      if (table.hand && table.hand.phase !== 'COMPLETE') table.closeAfterHand = true;
      else { this.closeTable(table, 'host_left'); return; }
    }

    if (table.seats.length === 0 && table.observers.size === 0) { this.closeTable(table, 'empty'); return; }
  ...
  sitOut(tableId: string, playerId: string, out: boolean): void {
    const table = this.require(tableId);
    const seat = table.seats.find(s => s.player.playerId === playerId);
    if (!seat) throw new TableError('NOT_SEATED', 'You are not seated at this table.');
    seat.sittingOut = out;
    this.pushTable(table);
    if (!out) this.maybeStartHand(table);
  }
  ...
  rebuy(tableId: string, playerId: string): void {
    ...
    const seat = table.seats.find(s => s.player.playerId === playerId);
    if (!seat) throw new TableError('NOT_SEATED', 'You are not seated at this table.');
    if (seat.stack > 0) throw new TableError('HAS_CHIPS', 'You still have chips.');
  ...
  setConnected(playerId: string, connected: boolean): void {
    for (const table of this.tablesFor(playerId)) this.markConnected(table, playerId, connected);
  }

  private markConnected(table: PokerTable, playerId: string, connected: boolean): void {
    const seat = table.seats.find(s => s.player.playerId === playerId);
    if (!seat || seat.connected === connected) {
      if (seat) seat.disconnectedAt = connected ? null : seat.disconnectedAt;
      return;
    }
  ...
  resume(playerId: string): { table: TableView }[] {
   ...
   */
  act(tableId: string, playerId: string, params: { handId: string; actionSeq: number; action: Action }): void {
    const table = this.require(tableId);
    const hand = table.hand;
    if (!hand || hand.phase === 'COMPLETE') throw new TableError('NO_HAND', 'There is no hand to act in.');
   ...
   */
  private maybeStartHand(table: PokerTable): void {
    if (table.status === 'closed') return;
    if (table.hand && table.hand.phase !== 'COMPLETE') return;
    if (this.startTimers.has(table.id)) return;
    if (this.playable(table).length < 2) { table.status = 'open'; return; }

    const handle = this.clock.setTimeout(() => {
      this.startTimers.delete(table.id);
      if (table.status === 'closed') return;
      if (table.hand && table.hand.phase !== 'COMPLETE') return;
      if (this.playable(table).length < 2) { table.status = 'open'; this.pushTable(table); return; }
      this.dealHand(table);
   ...
   */
  private playable(table: PokerTable): Seat[] {
    return table.seats.filter(s => !s.sittingOut && s.stack > 0);
  }

  private dealHand(table: PokerTable): void {
    ...
    table.hand = hand;
    for (const s of ready) s.handsPlayed += 1;
   ...
   */
  private runEngine(table: PokerTable, fn: (hand: HandState) => HandState): void {
  ...
  private afterEngine(table: PokerTable): void {
    const hand = table.hand;
    if (!hand) return;

    this.clearActionTimer(table.id);

    if (hand.phase === 'COMPLETE') { this.settle(table); return; }
  ...
  private settle(table: PokerTable): void {
    const hand = table.hand;
    if (!hand) return;
    ...
    for (const seat of table.seats) {
      ...
      if (inHand) seat.stack = inHand.stack;
      ...
      if (won > contributed) seat.handsWon += 1;
    }
    ...
    if (table.closeAfterHand) { this.closeTable(table, 'host_left'); return; }

    this.maybeStartHand(table);
  }

  private buildHistory(table: PokerTable, hand: HandState): HandHistory {
    return {
      handId: hand.handId,
  ...
  private startActionTimer(table: PokerTable, playerId: string): void {
    const seat = table.seats.find(s => s.player.playerId === playerId);
  ...
  private clearActionTimer(tableId: string): void {
    ...
    if (existing) { this.clock.clearTimeout(existing.handle); this.actionTimers.delete(tableId); }
  }

  private later(table: PokerTable, fn: () => void, ms: number): void {
    const handle = this.clock.setTimeout(() => {
  ...
  private clearTimers(tableId: string): void {
    ...
    if (start) { this.clock.clearTimeout(start); this.startTimers.delete(tableId); }
    for (const handle of this.otherTimers.get(tableId) ?? []) this.clock.clearTimeout(handle);
  ...
  closeTable(table: PokerTable, reason: string): void {
    ...
    this.later(table, () => this.tables.delete(table.id), 60_000);
    this.broadcastList();
  }

  /** Admin/host: close a table. Cannot change a result — there is no such call. */
  closeTableById(tableId: string, actorId: string, reason = 'closed_by_host'): void {
    const table = this.require(tableId);
    if (table.hostId !== actorId) throw new TableError('NOT_HOST', 'Only the host can close the table.');
    this.closeTable(table, reason);
  }

  /** Process shutdown: stop every timer so nothing fires into a dead process. */
  shutdown(): void {
    for (const id of this.tables.keys()) this.clearTimers(id);
  }
   ...
   */
  private pushTable(table: PokerTable): void {
    const viewers = [
  ...
  private emitToTable(table: PokerTable, event: string, payload: unknown): void {
  ...
  private reject(table: PokerTable, playerId: string, code: string, detail: Record<string, unknown>): void {
  ...
  private log(
    ...
    extra: { tableId?: string; handId?: string; detail?: Record<string, unknown> } = {},
  ): void {
    this.audit({ at: this.clock.now(), actorId, actorKind, event, ...extra });
  }

  private require(tableId: string): PokerTable {
    const table = this.tables.get(tableId);
    if (!table) throw new TableError('NO_TABLE', 'That table does not exist.');
    return table;
  }
}
