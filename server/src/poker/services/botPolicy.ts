...
import { parseCards, type Card } from '../engine/cards.js';
...
export function handStrength(hole: string[], board: string[]): number {
  ...
  if (board.length === 0) {
    if (own.length < 2) return 0.3;
    const [a, b] = own as [Card, Card];
    const high = Math.max(a.rank, b.rank);
    const low = Math.min(a.rank, b.rank);
    if (a.rank === b.rank) return Math.min(1, 0.55 + (a.rank - 2) / 24);   // a pair
 ...
 */
export function decide(spot: BotSpot): Action {
  ...
  if (legal.canCheck) {
    if (legal.canRaise && strength > 0.7 && roll < 0.45) return raiseTo(spot, 0.6);
    if (legal.canRaise && strength > 0.55 && roll < 0.15) return raiseTo(spot, 0.4);
    return { type: 'check' };
  }
  ...
  if (legal.canRaise && strength > 0.8 && roll < 0.35) return raiseTo(spot, 0.75);

  if (worthIt && legal.canCall) return { type: 'call' };
...
function raiseTo(spot: BotSpot, fraction: number): Action {
  const { legal } = spot;
