...
export function legalActions(ctx: BettingContext, seat: BettingSeat): LegalActions {
 ...
 */
export function validateAction(ctx: BettingContext, seat: BettingSeat, action: Action): ValidationError | null {
  const legal = legalActions(ctx, seat);
  if (seat.folded) return { code: 'FOLDED', message: 'You have folded this hand.' };
  if (seat.allIn) return { code: 'ALL_IN', message: 'You are already all-in.' };
  ...
  switch (action.type) {
    ...
    case 'raise': {
      if (!legal.canRaise) return { code: 'CANNOT_RAISE', message: 'Cannot raise now.' };
      const to = Math.floor(action.amount ?? 0);
      if (!Number.isFinite(to)) return { code: 'BAD_AMOUNT', message: 'Raise amount is not a number.' };
      if (to > legal.maxRaiseTo) return { code: 'RAISE_TOO_BIG', message: 'You do not have that many chips.' };
      if (to < legal.minRaiseTo) return { code: 'RAISE_TOO_SMALL', message: `Minimum raise is ${legal.minRaiseTo}.` };
      return null;
    }
    default:
