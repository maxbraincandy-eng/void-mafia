...
export class GroqProvider implements AIProvider {
  ...
  constructor() {
    ...
    if (this.pinned) console.log(`[Hermes/Groq] model pinned by GROQ_MODEL = ${this.pinned}`);
  }
  ...
  async listUsable(): Promise<string[]> {
  ...
  private async resolveModel(force = false): Promise<string> {
    if (!force && this.resolved) return this.resolved;
    if (!force && this.resolving) return this.resolving;
    ...
    this.resolving = (async () => {
      ...
      if (!this.resolved) throw new Error('Groq offers no usable chat model for this key');
  ...
  async chat(messages: AIMessage[], systemPrompt: string, maxTokens = 2048): Promise<AIResponse> {
    ...
    try {
      ...
      if (!gone) throw new Error(`ჰერმესი დროებით გათიშულია — ${e.message ?? 'Groq error'}`);
