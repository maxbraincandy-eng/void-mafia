  ...
  return (
    <div className="min-h-screen bg-neon-grid-animated scanlines flex flex-col items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        >
          {oauthError && (
            <p className="text-neon-red text-xs font-mono text-center mb-1">{oauthError}</p>
          )}

          {/* Google */}
          <button
            onClick={() => { setClickedProvider('google'); window.location.href = '/api/auth/google'; }}
            disabled={oauthLoading || isLoading}
            className="w-full flex items-center justify-center gap-3 py-3 rounded-2xl border transition-all active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed"
            style={{ background: 'rgba(255,255,255,0.06)', borderColor: 'rgba(255,255,255,0.18)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.06)' }}
          >
            {oauthLoading && clickedProvider === 'google' ? (
          ...
          {/* Facebook — disabled until Meta verification is complete */}
          <div
            className="w-full flex items-center justify-between gap-3 py-3 px-4 rounded-2xl border cursor-not-allowed select-none opacity-45"
            style={{ background: 'rgba(24,119,242,0.06)', borderColor: 'rgba(24,119,242,0.20)' }}
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-white/50 text-sm font-mono font-bold tracking-wide">{t.login.continueFacebook}</span>
            </div>
            <span className="text-[10px] font-mono text-white/30 tracking-widest uppercase shrink-0">მალე</span>
          </div>

          {/* Apple */}
          <button
            onClick={() => { setClickedProvider('apple'); window.location.href = '/api/auth/apple'; }}
            disabled={oauthLoading || isLoading}
            className="w-full flex items-center justify-center gap-3 py-3 rounded-2xl border transition-all active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed"
            style={{ background: 'rgba(255,255,255,0.05)', borderColor: 'rgba(255,255,255,0.15)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.06)' }}
          >
            {oauthLoading && clickedProvider === 'apple' ? (
              <span className="text-white/50 text-sm font-mono tracking-wide">{t.login.signingIn}</span>
            ) : (
              <>
                <svg viewBox="0 0 24 24" className="w-5 h-5 flex-shrink-0 fill-current text-white/85" xmlns="http://www.w3.org/2000/svg">
                  <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11"/>
                </svg>
                <span className="text-white/80 text-sm font-mono font-bold tracking-wide">{t.login.continueApple}</span>
              </>
            )}
          </button>
        </motion.div>
