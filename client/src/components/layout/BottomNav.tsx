...
export function BottomNav({ active, isMod, onChange, onMessagesClick }: Props) {
  ...
  // Split: tabs before MSG slot (rooms, clans) and after (profile, leaderboard, mod)
  const beforeMsg = visible.filter(t => t.id === 'rooms' || t.id === 'clans');
  const afterMsg  = visible.filter(t => t.id !== 'rooms' && t.id !== 'clans');

  const renderTab = (tab: typeof visible[0]) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-void/95 border-t border-white/5 backdrop-blur-xl">
      <div className="flex items-center justify-around max-w-lg mx-auto relative">
        {beforeMsg.map(renderTab)}

        {/* Messages button — now in position 3 */}
        <button
