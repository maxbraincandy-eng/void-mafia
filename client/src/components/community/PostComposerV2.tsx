    ...
    <div className="space-y-2">
      ...
      {options.map((opt, i) => (
        <div key={i} className="flex items-center gap-2">
          <TextInput value={opt} onChange={v => { const next = [...options]; next[i] = v; setOptions(next); }} placeholder={`Option ${i + 1}`} maxLength={100} />
          {options.length > 2 && (
            <button onClick={() => setOptions(options.filter((_, j) => j !== i))} className="text-white/30 hover:text-red-400/80 transition-colors font-mono text-sm flex-shrink-0">✕</button>
          )}
        </div>
      ))}
      {options.length < 4 && (
        <button onClick={() => setOptions([...options, ''])} className="font-mono text-[11px] text-white/40 hover:text-white/70 transition-colors">
    ...
    try {
      ...
      if (postType === 'poll') {
        data.poll = {
          ...
          options: pollOptions.filter(o => o.trim()).map((text, i) => ({ id: `opt_${i}`, text })),
          endsAt: null,
        };
      }
  ...
  return (
    <ModalShell onClose={onClose} accent="purple" maxWidthClass="max-w-lg">
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {TYPES.map(tp => (
          <button key={tp.id} onClick={() => setPostType(tp.id)}
            className="flex items-center gap-1 px-2.5 py-1 rounded-full font-mono text-[10px] uppercase tracking-wider transition-all"
            style={{
              background: postType === tp.id ? 'linear-gradient(135deg, rgba(155,0,255,0.28), rgba(0,245,255,0.16))' : 'rgba(255,255,255,0.03)',
      ...
      <div className="space-y-2 mb-4">
        ...
        {['movie_rec', 'series_rec', 'book_rec', 'music_rec'].includes(postType) && (
          <>
            ...
            <TextInput value={imageUrl} onChange={setImageUrl} placeholder={t.community.composer.recImagePh} />
          </>
        )}
        {postType === 'poll' && (
          <PollBuilder question={pollQuestion} setQuestion={setPollQuestion} options={pollOptions} setOptions={setPollOptions} />
        )}
      </div>
      ...
      <div className="flex items-center gap-2 mb-4">
        {(['public', 'friends_only'] as const).map(v => (
          <button key={v} onClick={() => setVisibility(v)}
            ...
            {v === 'public' ? '🌍' : '🔒'} {v === 'public' ? 'Public' : 'Friends'}
          </button>
        ))}
      </div>

      <button
        onClick={handlePost} disabled={!canPost || posting}
