 ...
 */
function ImageStack({ post }: { post: CommunityPostV2 }) {
  ...
  const frame = (src: string, key: number) => (
    <div key={key}
      ...
      style={{ scrollSnapAlign: 'center', maxHeight: 560 }}>
    ...
    >
          ...
          <div className="text-left min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              {isMrMax ? (
                ...
                // PlayerName draws the badge itself; it is suppressed here so the
                // two branches share ONE badge in one position — otherwise a
      ...
      {(post.imageUrl || post.gifUrl) && (
        <div className="relative" onClick={handleMediaTap} onDoubleClick={e => e.preventDefault()}>
          {post.imageUrl && <ImageStack post={post} />}
          {post.gifUrl && (
