...
import type { PhiloScenario, PhiloChoice, PhiloTally, PhiloEnding } from './types';

/**
 * Generic player for a philosophical thought experiment. Tracks the two-pole
 * tally + flags, then shows the scenario's computed ending (the player's
 * "philosophical position") with a meter between the scenario's two poles.
 */
export function PhilosophyPlayer({ scenario, onClose }: { scenario: PhiloScenario; onClose: () => void }) {
  ...
  const [tally, setTally] = useState<PhiloTally>({ a: 0, b: 0 });
  const [flags, setFlags] = useState<Record<string, string | boolean>>({});
  const [ending, setEnding] = useState<PhiloEnding | null>(null);

  const scene = sceneMap.get(sceneId) ?? null;

  const pick = (c: PhiloChoice) => {
    SFX.click();
    haptic('selection');
    const nextTally = { ...tally };
    if (c.lean === 'a') nextTally.a += 1;
    else if (c.lean === 'b') nextTally.b += 1;
  ...
  const bPct = Math.round((tally.b / total) * 100);

  return createPortal(
    <div
      className="fixed inset-0 z-[500] flex flex-col select-none"
      style={{ background: 'radial-gradient(ellipse 90% 60% at 50% 0%, #0f1330, #05060f 65%)', fontFamily: '"Space Grotesk", system-ui, sans-serif' }}
        ...
        <div className="max-w-xl mx-auto">
          {ending ? (
            <div className="text-center pt-8">
              ...
              <p className="text-[11px] tracking-[0.3em] uppercase mb-1" style={{ color: `${accent}88` }}>შენი ფილოსოფიური პოზიცია</p>
              <p className="text-2xl font-bold mb-5" style={{ color: ending.color, textShadow: `0 0 18px ${ending.color}55` }}>{ending.label}</p>

              {/* pole-A vs pole-B meter */}
              <div className="max-w-[340px] mx-auto mb-6">
                <div className="flex justify-between text-[11px] mb-1" style={{ color: 'rgba(255,255,255,0.45)' }}>
                  <span>{scenario.poleA} · {tally.a}</span>
                  <span>{tally.b} · {scenario.poleB}</span>
                </div>
                <div className="h-2 rounded-full overflow-hidden flex" style={{ background: 'rgba(255,255,255,0.08)' }}>
                  <div style={{ width: `${100 - bPct}%`, background: '#e8dcc8' }} />
                  <div style={{ width: `${bPct}%`, background: accent }} />
                </div>
              </div>
