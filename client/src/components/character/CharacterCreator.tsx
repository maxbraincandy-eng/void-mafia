...
import {
  ...
  SKIN_TONES, HAIR_COLORS, EYE_COLORS, CLOTH_COLORS, LIP_COLORS, SHADOW_COLORS,
  HAIR_STYLES, BEARD_STYLES, TOP_STYLES, BOTTOM_STYLES, SHOE_STYLES, BUILDS, GLASSES, HATS,
} from './spec';
...
function randomize(gender: Gender): CharacterSpec {
  ...
  return {
    ...
    hair: rand(HAIR_STYLES).id, hairColor: rand(HAIR_COLORS), eyeColor: rand(EYE_COLORS), browColor: rand(HAIR_COLORS.slice(0, 6)),
    beard: gender === 'male' ? rand(BEARD_STYLES).id : 'none', beardColor: rand(HAIR_COLORS.slice(0, 6)),
    lipstick: gender === 'female' ? rand(LIP_COLORS) : '', eyeshadow: gender === 'female' ? rand(SHADOW_COLORS) : '', blush: gender === 'female' && Math.random() < 0.5,
    top: rand(TOP_STYLES).id, topColor: rand(CLOTH_COLORS), bottom: rand(BOTTOM_STYLES).id, bottomColor: rand(CLOTH_COLORS),
    shoes: rand(SHOE_STYLES).id, shoeColor: rand(CLOTH_COLORS), glasses: rand(GLASSES).id, hat: rand(HATS).id, hatColor: rand(CLOTH_COLORS),
  };
}
...
function Options({ cat, spec, patch }: { cat: Cat; spec: CharacterSpec; patch: (p: Partial<CharacterSpec>) => void }) {
  ...
  if (cat === 'style') return spec.gender === 'male' ? (<>
    ...
    <Section title="ჩრდილი"><Swatches colors={SHADOW_COLORS} value={spec.eyeshadow} onPick={c => patch({ eyeshadow: c })} allowNone /></Section>
    <Section title="ღაწვები"><Chips items={[{ id: 'off', label: 'გამორთ.' }, { id: 'on', label: 'ჩართ.' }]} value={spec.blush ? 'on' : 'off'} onPick={v => patch({ blush: v === 'on' })} /></Section>
  </>);
