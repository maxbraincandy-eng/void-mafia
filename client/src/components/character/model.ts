...
export function buildCharacter(spec: CharacterSpec): CharacterModel {
  ...
  const update = (dt: number, e: number) => {
    ...
    if (holdPose === 'titanic') {
      ...
      // The one UNDERNEATH: an ordinary seated posture (the engine now applies
      // the same hip drop as a normal seat, so the hips land on the cushion),
      // with both arms brought forward around the partner sitting on the lap.
      const k = Math.min(1, dt * 10);
      legGroups.forEach(l => { l.rotation.x += (-1.48 - l.rotation.x) * k; });
      kneeGroups.forEach(kn => { kn.rotation.x += (1.42 - kn.rotation.x) * k; });
      armGroups[0].rotation.x += (-1.15 - armGroups[0].rotation.x) * k;
      armGroups[0].rotation.z += (0.46 - armGroups[0].rotation.z) * k;
      armGroups[1].rotation.x += (-1.15 - armGroups[1].rotation.x) * k;
      armGroups[1].rotation.z += (-0.46 - armGroups[1].rotation.z) * k;
      torso.rotation.z = Math.sin(e * 1.05) * 0.025;
    } else if (holdPose === 'lapTop') {
      // The one ON THE LAP: same seated posture, facing the SAME way as the
      // partner (both watching the screen), knees a little higher because the
      // thighs rest on theirs, leaning back against their chest.
      const k = Math.min(1, dt * 10);
      legGroups.forEach(l => { l.rotation.x += (-1.36 - l.rotation.x) * k; });
      kneeGroups.forEach(kn => { kn.rotation.x += (1.2 - kn.rotation.x) * k; });
      armGroups[0].rotation.x += (-0.62 - armGroups[0].rotation.x) * k;
      armGroups[0].rotation.z += (0.3 - armGroups[0].rotation.z) * k;
      armGroups[1].rotation.x += (-0.62 - armGroups[1].rotation.x) * k;
