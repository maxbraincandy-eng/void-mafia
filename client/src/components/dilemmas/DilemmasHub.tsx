...
import { DILEMMAS, DILEMMA_TEASERS } from './registry';

/**
 * Full-screen hub for the Dilemmas category — same shape as Premium Worlds:
 * a header and a scrollable list of dilemma cards. Playable ones open the
 * DilemmaPlayer; upcoming ones show greyed as "მალე". New dilemmas appear
 * here automatically by adding them to the registry.
 */
export function DilemmasHub({ onClose }: { onClose: () => void }) {
  const [openId, setOpenId] = useState<string | null>(null);
  const openScenario = openId ? DILEMMAS.find(d => d.id === openId) ?? null : null;

  return createPortal(
    <div
      className="fixed inset-0 z-[490] flex flex-col select-none"
      style={{ background: 'radial-gradient(ellipse 100% 55% at 50% 0%, #14132e, #06060f 60%)' }}
    ...
    >
      ...
      <div className="flex items-start justify-between px-5 pt-[calc(env(safe-area-inset-top,0px)+16px)] pb-3 flex-shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-wide text-xl" style={{ color: '#a9b6ff' }}>დილემები</h1>
            <p className="font-mono text-[12px] text-white/40 leading-tight">მორალური არჩევანი · უპასუხე შენს სინდისს</p>
          </div>
        </div>
      ...
      {openScenario && <DilemmaPlayer scenario={openScenario} onClose={() => setOpenId(null)} />}
    </div>,
    document.body,
  );
}
