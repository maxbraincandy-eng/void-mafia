/**
 * useSpyfallVoice — WebRTC Push-to-Talk voice for ჯაშუში (Spyfall) matches.
 * Mesh fallback used when LiveKit is not configured; mirrors useUnoVoice.
 */
...
export interface SpyfallVoiceState {
  matchId: string | null;
...
const INITIAL: SpyfallVoiceState = {
  matchId: null,
  status: 'disconnected',
  joined: false,
  isTalking: false,
...
let _state: SpyfallVoiceState = { ...INITIAL };
const _subscribers = new Set<(s: SpyfallVoiceState) => void>();
let _joining = false;
let _wantsToTalk = false;

function _patch(partial: Partial<SpyfallVoiceState>) {
...
async function _createAndJoin(matchId: string, listenOnly: boolean): Promise<void> {
  ...
  (socket as any).emit('spy:voice-join', { matchId }, async (res: any) => {
    _joining = false;
    if (!_session || _session !== session) return;
    ...
    for (const { socketId, name } of existingPeers as Array<{ socketId: string; name: string }>) {
      session.createPeerConnection(socketId, name, (candidate) => {
        (socket as any).emit('spy:voice-ice', { to: socketId, candidate });
      });
      try {
        const offer = await session.createOffer(socketId);
        (socket as any).emit('spy:voice-offer', { to: socketId, sdp: offer });
      } catch (e: any) {
        log('spyfall voice offer failed for', socketId, ':', e.message);
      }
    }
    _patch({ peers: session.getPeers() });
    ...
    if (_wantsToTalk && !listenOnly) {
      ...
      (socket as any).emit('spy:ptt-start', { matchId });
    }
  });
}
...
function _leave(): void {
  ...
  if (matchId) {
    if (_state.isTalking) (socket as any).emit('spy:ptt-stop', { matchId });
    (socket as any).emit('spy:voice-leave', { matchId });
  }
  ...
  if (_session.isListenOnly()) {
    try {
      await _session.upgradeToSpeaker((peerId, offer) => {
        (socket as any).emit('spy:voice-offer', { to: peerId, sdp: offer });
      });
    } catch {
...
function _stopTalking(matchId: string): void {
  ...
  (socket as any).emit('spy:ptt-stop', { matchId });
}

(socket as any).on('spy:voice-peer-joined', ({ socketId, name }: { socketId: string; name: string }) => {
  const s = _session;
  if (!s) return;
...
(socket as any).on('spy:voice-peer-left', ({ socketId }: { socketId: string }) => {
...
(socket as any).on('spy:voice-offer', async ({ from, sdp }: { from: string; sdp: RTCSessionDescriptionInit }) => {
  const s = _session;
  if (!s) return;
  try {
    ...
    const answer = await s.handleOffer(from, existingName, sdp, (candidate) => {
      (socket as any).emit('spy:voice-ice', { to: from, candidate });
    });
    (socket as any).emit('spy:voice-answer', { to: from, sdp: answer });
    _patch({ peers: s.getPeers() });
  } catch (e: any) {
    log('spyfall voice offer handling failed:', e.message);
  }
});

(socket as any).on('spy:voice-answer', async ({ from, sdp }: { from: string; sdp: RTCSessionDescriptionInit }) => {
  try { await _session?.handleAnswer(from, sdp); } catch (e: any) { log('spyfall voice answer failed:', e.message); }
});

(socket as any).on('spy:voice-ice', async ({ from, candidate }: { from: string; candidate: RTCIceCandidateInit }) => {
  try { await _session?.addIceCandidate(from, candidate); } catch {}
});

(socket as any).on('spy:ptt-state', ({ socketId, speaking }: { socketId: string; speaking: boolean }) => {
  const next = speaking
...
export function useSpyfallVoice() {
  const [state, setLocalState] = useState<SpyfallVoiceState>({ ..._state });
...
export function leaveSpyfallVoice(): void {
  _leave();
}
