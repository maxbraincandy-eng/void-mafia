...
import type { SpatialListener, SpatialPeer } from '@/components/backrooms/spatialAudio';
import { tNow } from '@/store/langStore';

// ── Premium Worlds voice ───────────────────────────────────────────────
...
//
// Audio plays FLAT through WebRTCSession's own <audio> elements — the same
// reliable path the 2D Space voice uses. An earlier version routed remote audio
// through a distance-based Web-Audio spatializer, but its gain started at 0 and
// only became audible if the per-tick position update ran with fresh peer
// coordinates; any hiccup (or a peer whose position wasn't syncing) left the
// graph at gain 0 → players heard nothing even though the mesh was connected.
// Flat audio is never silent, which is what matters most in a social world.
...
async function _join(retriesLeft = 4): Promise<void> {
  ...
  catch { session.destroy(); _session = null; _joining = false; _patch({ status: 'failed', error: tNow().misc.micDenied }); return; }
...
/**
 * Kept for API compatibility with the world loop / tap handlers. Audio now
 * plays flat through WebRTCSession's <audio> elements (no suspended Web-Audio
 * graph to resume), so this is a no-op.
 */
export function resumeWorldVoiceAudio(): void { /* flat audio needs no resume */ }

/** Kept for API compatibility — spatialisation was removed (see file header). */
export function applyWorldSpatial(_listener: SpatialListener, _peers: SpatialPeer[]): void { /* no-op */ }
...
(socket as any).on('disconnect', () => { if (_session) { _session.destroy(); _reset(); } });
