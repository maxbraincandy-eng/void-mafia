/**
 * useWWWVoice — WebRTC Push-to-Talk voice for "What? Where? When?" matches.
 *
...
async function _createAndJoin(matchId: string, listenOnly: boolean): Promise<void> {
  ...
  (socket as any).emit('www:voice-join', { matchId }, async (res: any) => {
    ...
    if (_wantsToTalk && !listenOnly) {
      ...
      (socket as any).emit('www:ptt-start', { matchId });
    }
...
function _leave(): void {
  ...
  if (matchId) {
    if (_state.isTalking) {
      (socket as any).emit('www:ptt-stop', { matchId });
    }
...
(socket as any).on('www:ptt-state', ({ socketId, speaking }: { socketId: string; speaking: boolean }) => {
  const next = speaking
