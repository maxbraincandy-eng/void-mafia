/**
 * useWWWVoice — WebRTC Push-to-Talk voice for WWW (What? Where? When?) matches.
 *
 * Isolated from Mafia game-room voice, Community lounge voice, and Checkers/Ludo voice.
...
export interface WWWVoiceState {
  matchId: string | null;
...
let _state: WWWVoiceState = { ...INITIAL };
const _subscribers = new Set<(s: WWWVoiceState) => void>();
let _joining = false;
let _wantsToTalk = false;

function _patch(partial: Partial<WWWVoiceState>) {
...
async function _createAndJoin(matchId: string, listenOnly: boolean): Promise<void> {
  ...
  (socket as any).emit('www:voice-join', { matchId }, async (res: any) => {
    _joining = false;
    ...
    for (const { socketId, name } of existingPeers as Array<{ socketId: string; name: string }>) {
      session.createPeerConnection(socketId, name, (candidate) => {
        (socket as any).emit('www:voice-ice', { to: socketId, candidate });
      });
      try {
        const offer = await session.createOffer(socketId);
        (socket as any).emit('www:voice-offer', { to: socketId, sdp: offer });
      } catch (e: any) {
        log('www voice offer failed for', socketId, ':', e.message);
      }
    }
    _patch({ peers: session.getPeers() });
    ...
    if (_wantsToTalk && !listenOnly) {
      ...
      (socket as any).emit('www:voice-start-talk', { matchId });
    }
  });
}
...
function _leave(): void {
  ...
  if (matchId) {
    if (_state.isTalking) {
      (socket as any).emit('www:voice-stop-talk', { matchId });
    }
    (socket as any).emit('www:voice-leave', { matchId });
  }
  ...
  if (_session.isListenOnly()) {
    try {
      await _session.upgradeToSpeaker((peerId, offer) => {
        (socket as any).emit('www:voice-offer', { to: peerId, sdp: offer });
      });
    } catch {
...
(socket as any).on('www:voice-peer-joined', ({ socketId, name }: { socketId: string; name: string }) => {
  const s = _session;
  if (!s) return;
...
(socket as any).on('www:voice-peer-left', ({ socketId }: { socketId: string }) => {
...
(socket as any).on('www:voice-offer', async ({ from, sdp }: { from: string; sdp: RTCSessionDescriptionInit }) => {
  const s = _session;
  if (!s) return;
  try {
    ...
    const answer = await s.handleOffer(from, existingName, sdp, (candidate) => {
      (socket as any).emit('www:voice-ice', { to: from, candidate });
    });
    (socket as any).emit('www:voice-answer', { to: from, sdp: answer });
    _patch({ peers: s.getPeers() });
  } catch (e: any) {
    log('www voice offer handling failed:', e.message);
  }
});

(socket as any).on('www:voice-answer', async ({ from, sdp }: { from: string; sdp: RTCSessionDescriptionInit }) => {
  try { await _session?.handleAnswer(from, sdp); } catch (e: any) { log('www voice answer failed:', e.message); }
});

(socket as any).on('www:voice-ice', async ({ from, candidate }: { from: string; candidate: RTCIceCandidateInit }) => {
  try { await _session?.addIceCandidate(from, candidate); } catch {}
});

(socket as any).on('www:voice-peer-state', ({ socketId, speaking }: { socketId: string; speaking: boolean }) => {
  const next = speaking
...
export function useWWWVoice() {
  const [state, setLocalState] = useState<WWWVoiceState>({ ..._state });
...
export function leaveWWWVoice(): void {
  _leave();
}
